...
export function BrowseSupplierModal({ isOpen, onClose, onSelect }: Props) {
  ...
  const fetchSupplier = async (search: string) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/master/supplier?keyword=${search}`);
