...
export function BrowseSalesModal({ isOpen, onClose, onSelect }: Props) {
  ...
  const fetchSales = async (search: string) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/master/sales?keyword=${search}`);
          ...
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input 
                ...
                autoFocus
                className="w-full pl-9 pr-4 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-all"
              />
            </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-[300px] p-4 bg-slate-50/50">
          {loading ? (
            <div className="flex flex-col items-center justify-center h-full text-slate-500 space-y-2">
              ...
              <span>Memuat data sales...</span>
            </div>
          ) : items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-slate-500 space-y-2">
              <ShoppingCart className="h-10 w-10 text-slate-300" />
              <span>Tidak ada sales ditemukan</span>
            </div>
          ) : (
              ...
              {items.map(item => (
                ...
                >
                  <div className="flex flex-col">
                    <span className="text-sm font-bold text-slate-900 group-hover:text-indigo-700 transition-colors uppercase tracking-tight">{item.nama}</span>
                    <span className="text-xs font-mono text-slate-500">{item.kode}</span>
                  </div>
                  <div className="flex items-center gap-3">
                     <button className="px-3 py-1.5 text-[10px] font-black uppercase tracking-wider text-white bg-indigo-600 rounded opacity-0 group-hover:opacity-100 transition-opacity">
                       Pilih
                     </button>
                  </div>
                </div>
