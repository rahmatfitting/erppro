...
export function BrowseBarangModal({ isOpen, onClose, onSelect, filterKategori }: Props) {
  ...
  const fetchBarang = async (search: string) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/master/barang?keyword=${search}&kategori=${filterKategori || ''}`);
