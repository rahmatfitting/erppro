...
export function BrowseCustomerModal({ isOpen, onClose, onSelect }: Props) {
  ...
  const fetchCustomer = async (search: string) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/master/customer?keyword=${search}`);
        ...
        <div className="flex-1 overflow-y-auto min-h-[300px] p-4 bg-slate-50/50">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-slate-500 space-y-2">
              ...
              <span>Tidak ada customer ditemukan</span>
            </div>
          ) : (
