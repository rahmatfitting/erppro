...
type ItemReceipt = {
  id: number;
  nomortdbelipenerimaan: number;
  nomorthbelipenerimaan: number;
...
type Props = {
  isOpen: boolean;
  onClose: () => void;
  onSelect: (item: ItemReceipt) => void;
  pbNomor: number | null;
  pbId: string;
};

export function BrowseBarangPenerimaanModal({ isOpen, onClose, onSelect, pbNomor, pbId }: Props) {
  const [items, setItems] = useState<ItemReceipt[]>([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (isOpen && (pbNomor || pbId)) {
        fetchReceiptDetails(pbNomor || pbId);
    }
  }, [isOpen, pbNomor, pbId]);

  const fetchReceiptDetails = async (id: number | string) => {
    setLoading(true);
    try {
      ...
      if (json.success && json.data.items) {
          ...
          const mappedItems = json.data.items.map((it: any) => ({
              ...it,
              nomortdbelipenerimaan: it.id,
              nomorthbelipenerimaan: it.nomorthbelipenerimaan || id,
          }));
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-slate-50">
          <div className="flex flex-col">
            <div className="flex items-center gap-2 text-lg font-bold text-slate-800">
              ...
              Pilih Barang dari LPB
            </div>
            <span className="text-xs text-slate-500 font-medium ml-7">Ref: {pbId}</span>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto min-h-[300px] p-4 bg-white/50">
          {loading ? (
            <div className="flex flex-col items-center justify-center h-48 text-slate-500 space-y-2">
              ...
              <span>Memuat detail item LPB...</span>
            </div>
          ) : items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-48 text-slate-500 space-y-2">
              <Package className="h-10 w-10 text-slate-300" />
              <span>Tidak ada item tersedia di LPB ini</span>
            </div>
          ) : (
              ...
              <table className="w-full text-left text-sm text-slate-600">
                <thead className="bg-slate-50 text-xs uppercase text-slate-700 font-bold">
                  <tr>
                    ...
                    <th className="px-4 py-3 w-24 text-right">Diterima</th>
                  ...
                  {items.map(item => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right font-black text-slate-900">
                        {item.jumlahDiterima || item.jumlah}
                      </td>
