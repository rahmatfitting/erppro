...
type Props = {
  ...
  onClose: () => void;
  onSelect: (po: PO) => void;
  openReceiptOnly?: boolean;
};

export function BrowsePOModal({ isOpen, onClose, onSelect, openReceiptOnly }: Props) {
  ...
  const fetchPO = async (search: string) => {
    setLoading(true);
    try {
      // Fetch POs, optionally filtered by those that still have items to receive
      const url = `/api/pembelian/order?keyword=${search}${openReceiptOnly ? '&openreceipt=true' : ''}`;
        ...
        <div className="flex-1 overflow-y-auto min-h-[300px] p-4 bg-slate-50/50">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-slate-500 space-y-2">
              ...
              <span>Tidak ada Order Beli ditemukan</span>
            </div>
          ) : (
