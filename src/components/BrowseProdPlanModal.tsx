...
import { Search, X, Loader2, Activity, Calendar } from "lucide-react";
...
type Props = {
  isOpen: boolean;
  onClose: () => void;
  onSelect: (plan: ProdPlan) => void;
};

export function BrowseProdPlanModal({ isOpen, onClose, onSelect }: Props) {
  const [keyword, setKeyword] = useState("");
  const [items, setItems] = useState<ProdPlan[]>([]);
  ...
  const fetchPlans = async (search: string) => {
    setLoading(true);
    try {
      // Hanya ambil plan yang statusnya belum Finished
      const res = await fetch(`/api/ppic/prodplan?keyword=${search}`);
      const json = await res.json();
      if (json.success) {
        // Filter di client jika API belum mendukung filter status ganda
        setItems(json.data.filter((p: any) => p.status !== 'Finished'));
      }
    } catch (e) {
      console.error(e);
    } finally {
        ...
        <div className="flex-1 overflow-y-auto min-h-[300px] p-4 bg-slate-50/30 dark:bg-slate-950/30">
          {loading ? (
            <div className="flex flex-col items-center justify-center h-full text-slate-500 space-y-2">
              ...
              <span>Memuat data plan...</span>
            </div>
          ) : items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-slate-500 space-y-2">
              <Activity className="h-10 w-10 text-slate-300" />
              <span>Tidak ada plan aktif ditemukan</span>
            </div>
          ) : (
