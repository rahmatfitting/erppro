...
import { Plus, Search, FileText, Trash2, CalendarDays, Loader2, MoreHorizontal } from "lucide-react";
...
export default function StokOpnameList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/stok/opname?${queryParams.toString()}`);
      const result = await response.json();
      if (result.success) {
        setData(result.data);
      }
    } catch (error) {
      console.error("Failed to fetch Stok Opname list:", error);
    } finally {
      setLoading(false);
    }
  }, [filters]);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1 dark:text-slate-400">
            Kelola penyesuaian stok barang berdasarkan hasil cek fisik di gudang.
          </p>
        </div>
        <Link 
          href="/stok/opname/create" 
          className="inline-flex items-center justify-center gap-2 rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 transition-colors"
        >
        ...
        <div className="flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1 w-full space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Kode SO atau Keterangan..."
          ...
          <div className="w-full md:w-48 space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-md border border-slate-200 bg-white pl-9 pr-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
              />
            </div>
          </div>
          <div className="w-full md:w-48 space-y-1.5">
          ...
          <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300 min-w-[800px]">
            <thead className="bg-slate-50 text-xs uppercase text-slate-700 dark:bg-slate-950 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold">Jenis Penyesuaian</th>
                <th className="px-6 py-4 font-semibold text-center">Items</th>
                <th className="px-6 py-4 font-semibold text-center status">Status</th>
                <th className="px-6 py-4 font-semibold text-center w-32">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/80">
              {loading ? (
                <tr><td colSpan={7} className="px-6 py-12 text-center"><Loader2 className="h-8 w-8 animate-spin mx-auto text-indigo-500" /></td></tr>
              ) : data.map((item) => (
                <tr key={item.nomor} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/50 group">
                  ...
                  <td className="px-6 py-4 font-medium">{item.gudang_nama}</td>
                  <td className="px-6 py-4">{item.penyesuaian_nama || '-'}</td>
                  <td className="px-6 py-4 text-center">{item.itemsCount}</td>
                  <td className="px-6 py-4 text-center">
                    <span className={cn("px-2.5 py-1 rounded-full text-xs font-medium border", getStatusDisplay(item.status_aktif, item.status_disetujui).color)}>
                      {getStatusDisplay(item.status_aktif, item.status_disetujui).label}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Link href={`/stok/opname/${item.kode}`} className="p-1.5 text-slate-400 hover:text-indigo-600 bg-white border border-slate-200 rounded shadow-sm dark:bg-slate-900 dark:border-slate-800"><Search className="h-4 w-4" /></Link>
                      {item.status_aktif === 1 && item.status_disetujui === 0 && (
                        <button 
                          onClick={() => handleAction(item.kode, 'approve')} 
                          disabled={isPending}
                          className="p-1.5 text-slate-400 hover:text-emerald-600 bg-white border border-slate-200 rounded shadow-sm dark:bg-slate-900 dark:border-slate-800 disabled:opacity-50"
                        >
                      ...
                      {item.status_aktif === 1 && item.status_disetujui === 0 && (
                         <button 
                           ...
                           disabled={isPending}
                           className="p-1.5 text-slate-400 hover:text-red-600 bg-white border border-slate-200 rounded shadow-sm dark:bg-slate-900 dark:border-slate-800 disabled:opacity-50"
                         >
