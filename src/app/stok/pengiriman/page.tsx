...
import { Plus, Search, Truck, Trash2, CalendarDays, Loader2, ArrowRight } from "lucide-react";
import { cn } from "@/lib/utils";

export default function TransferGudangList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/stok/pengiriman?${queryParams.toString()}`);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
            <Truck className="h-6 w-6 text-sky-600 dark:text-sky-500" />
            Transfer Antar Gudang
          </h2>
          <p className="text-sm text-slate-500 mt-1 dark:text-slate-400">
            Kirim dan terima barang antar lokasi gudang.
          </p>
        </div>
        <Link 
          href="/stok/pengiriman/create" 
          className="inline-flex items-center justify-center gap-2 rounded-md bg-sky-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-sky-700 transition-colors"
        >
          ...
          <div className="flex-1 w-full space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-md border border-slate-200 bg-slate-50 pl-9 pr-3 py-2 text-sm outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
              />
            </div>
          </div>
          ...
          <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300 min-w-[800px]">
             <thead className="bg-slate-50 text-xs uppercase text-slate-700 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800">
               <tr>
                 ...
                 <th className="px-6 py-4 font-semibold">Gudang Asal</th>
                 <th className="px-6 py-4 font-semibold text-center w-8 text-slate-300"><ArrowRight className="h-4 w-4" /></th>
             ...
             <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
               {loading ? (
                 <tr><td colSpan={8} className="px-6 py-12 text-center"><Loader2 className="h-8 w-8 animate-spin mx-auto text-sky-500" /></td></tr>
               ) : data.map((item) => (
                 <tr key={item.nomor} className="hover:bg-slate-50/70 group">
                   <td className="px-6 py-4 font-bold text-sky-600 uppercase">{item.kode}</td>
                   <td className="px-6 py-4">{new Date(item.tanggal).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' })}</td>
                   <td className="px-6 py-4 font-medium text-slate-900">{item.gudang_asal_nama}</td>
                   <td className="px-6 py-4 text-center"><ArrowRight className="h-3 w-3 text-slate-300" /></td>
                   <td className="px-6 py-4 font-medium text-slate-900">{item.gudang_tujuan_nama}</td>
                   ...
                   <td className="px-6 py-4 text-center">
                    <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Link href={`/stok/pengiriman/${item.kode}`} className="p-1.5 text-slate-400 hover:text-sky-600 bg-white border border-slate-200 rounded shadow-sm"><Search className="h-4 w-4" /></Link>
