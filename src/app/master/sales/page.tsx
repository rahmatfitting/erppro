...
import { Plus, Search, Trash2, Users, Loader2, Edit, AlertCircle, Printer } from "lucide-react";
...
export default function MasterSalesPage() {
  ...
  const fetchData = async (keyword = "") => {
    setLoading(true);
    try {
      const res = await fetch(`/api/master/sales?keyword=${keyword}`);
  ...
  const handleExport = async (startDate: string, endDate: string, exportType: 'view' | 'pdf' | 'excel' | 'pivot') => {
    try {
      ...
      const response = await fetch(`/api/master/sales?${queryParams.toString()}`);
      ...
      if (result.success) {
        ...
        const config = {
          ...
          fileName: `Master_Sales_${startDate}_${endDate}`,
  ...
  const handleToggleStatus = async (id: string, currentStatus: number) => {
    if (!confirm(`Apakah Anda yakin ingin ${currentStatus === 1 ? 'menonaktifkan' : 'mengaktifkan'} sales ini?`)) return;
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Kelola data staf penjualan atau representatif.
          </p>
        </div>
                  ...
                  <tr key={item.kode} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Link href={`/master/sales/${item.kode}?mode=view`} title="View" className="text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
                          <Search className="h-4 w-4" />
                        </Link>
                        <Link href={`/master/sales/${item.kode}?mode=edit`} title="Edit" className="text-slate-400 hover:text-amber-600 dark:hover:text-amber-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
                          <Edit className="h-4 w-4" />
                        </Link>
                        {item.status_aktif === 1 ? (
                           <button onClick={() => handleToggleStatus(item.kode, 1)} title="Nonaktifkan" className="text-slate-400 hover:text-red-600 dark:hover:text-red-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
                             <Trash2 className="h-4 w-4" />
                           </button>
                        ) : (
                           <button onClick={() => handleToggleStatus(item.kode, 0)} title="Aktifkan" className="text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
