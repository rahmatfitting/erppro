...
import { Plus, Search, Layers, FileEdit, Trash2, Printer } from "lucide-react";
...
export default function SatuanList() {
  ...
  const fetchData = useCallback(async (pageNum: number, isLoadMore = false) => {
    ...
    try {
      ...
      const response = await fetch(`/api/master/satuan?${queryParams.toString()}`);
  ...
  const handleAction = async (id: string, action: 'delete') => {
    if (!confirm(`Apakah Anda yakin ingin menghapus data satuan ini?`)) return;
    try {
      ...
      if (result.success) {
         ...
         alert(result.error || "Gagal menghapus satuan");
      }
    } catch (error) {
    ...
    try {
      ...
      if (result.success) {
        const exportColumns = [
          ...
          { header: "Nama Satuan", key: "nama" }
        ];

        const config = {
          title: "Master Data Satuan",
          subtitle: `Periode Input: ${startDate} s/d ${endDate}`,
          fileName: `Master_Satuan_${startDate}_${endDate}`,
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1 dark:text-slate-400">
            Kelola data satuan barang (Unit of Measure) seperti Pcs, Kg, Box, dll.
          </p>
        </div>
          ...
          <div className="flex-1 w-full space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-md border border-slate-200 bg-slate-50 pl-9 pr-3 py-2 text-sm outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 dark:border-slate-800 dark:bg-slate-950 dark:text-white"
              />
            </div>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 overflow-hidden">
          ...
          <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300 relative">
            <thead className="bg-slate-50 text-xs uppercase text-slate-700 dark:bg-slate-950 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold">Nama Satuan</th>
                <th className="px-6 py-4 font-semibold w-48">Dibuat Pada</th>
                ...
                <tr key={item.nomor} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-center">
                    <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Link href={`/master/satuan/${item.nomor}`} title="View" className="text-slate-400 hover:text-purple-600 dark:hover:text-purple-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
                ...
                <tr>
                   <td colSpan={4} className="px-6 py-12 text-center text-slate-500 dark:text-slate-400">
                      <div className="flex flex-col items-center justify-center">
                         ...
                         <p>Tidak ada Master Satuan yang ditemukan.</p>
                      </div>
                   </td>
                </tr>
              )}
        ...
        {hasMore && (
           <div ref={observerTarget} className="p-6 flex justify-center items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
             {loading ? (
                <>
                  <div className="h-4 w-4 animate-spin rounded-full border-2 border-purple-600 object-top border-t-transparent dark:border-purple-500"></div>
                  Memuat data...
                </>
             ) : (
