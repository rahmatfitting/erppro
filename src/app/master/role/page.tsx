...
import { Plus, Search, Settings, Edit, Loader2, AlertCircle, Ban, Users } from "lucide-react";
...
export default function RoleIndex() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/master/role?keyword=${keyword}`);
  ...
  const handleDeactivate = async (id: string, name: string) => {
    if (!confirm(`Apakah Anda yakin ingin menonaktifkan grup ${name}?`)) return;
    ...
    try {
      ...
      if (json.success) {
        ...
        alert(json.error || "Gagal menonaktifkan grup");
      }
    } catch (err: any) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1 dark:text-slate-400">
            Kelola grup pengguna dan otoritas akses menu sistem
          </p>
        </div>
        <Link 
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-slate-800">
              <tr>
                ...
                <th className="px-6 py-3 font-semibold w-32">Kode</th>
              ...
              {loading ? (
                ...
                data.map((row) => (
                  <tr key={row.kode} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-6 py-3 text-slate-900 font-semibold">{row.nama}</td>
                    <td className="px-6 py-3 text-slate-500 text-xs truncate max-w-[300px]" title={row.keterangan}>{row.keterangan || '-'}</td>
                    <td className="px-6 py-3">
                      <div className="flex items-center justify-center gap-2">
                        <Link 
                          href={`/master/role/${row.nomor}`}
