...
export default function CustomerDetail() {
  ...
  const fetchCustomer = async () => {
    setFetching(true);
    try {
      const res = await fetch(`/api/master/customer/${id}`);
      ...
      if (data.success) {
        setFormData({
          ...
          kontak_person: data.data.kontak_person || "",
  ...
  const handleToggleStatus = async () => {
    if (!confirm(`Apakah Anda yakin ingin ${formData.status_aktif === 1 ? 'menonaktifkan' : 'mengaktifkan'} customer ini?`)) return;
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
             <Link href="/master/customer" className="text-sm font-medium text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-300 transition-colors">
               Master Customer
             </Link>
             <span className="text-slate-300 dark:text-slate-600">/</span>
             <span className="text-sm font-medium text-slate-900 dark:text-white">Detail Customer</span>
          </div>
          ...
          {!isEdit ? (
            ...
            <>
              <button 
                onClick={() => {
                  setIsEdit(false);
                  fetchCustomer(); // fetch again to reset data
                }}
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  className={cn(
                     "w-full rounded-md px-3 py-2 text-sm outline-none transition-colors resize-none",
                     !isEdit 
                        ? "border border-slate-200 bg-slate-50 text-slate-700 cursor-not-allowed dark:border-slate-800 dark:bg-slate-950/50 dark:text-slate-300" 
