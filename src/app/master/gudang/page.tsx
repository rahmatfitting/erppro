...
import { Plus, Search, Home, FileEdit, Trash2, Printer, Loader2, AlertCircle } from "lucide-react";
...
export default function MasterGudangPage() {
  ...
  const fetchData = async (keyword = "") => {
    ...
    setError("");
    try {
      const res = await fetch(`/api/master/gudang?keyword=${keyword}`);
      const json = await res.json();
      if (json.success) {
        setData(json.data);
        setHasMore(json.data.length === ITEMS_PER_PAGE); // Assuming pagination is implemented
      } else {
        setError(json.error);
      }
    } catch (err) {
  ...
  const handleToggleStatus = async (id: string, currentStatus: number) => {
    if (!confirm(`Apakah Anda yakin ingin ${currentStatus === 1 ? 'menonaktifkan' : 'mengaktifkan'} gudang ini?`)) return;
  ...
  const handleExport = async (startDate: string, endDate: string, exportType: 'view' | 'pdf' | 'excel' | 'pivot') => {
    try {
      ...
      const response = await fetch(`/api/master/gudang?${queryParams.toString()}`);
      ...
      if (result.success) {
        ...
        const config = {
          ...
          fileName: `Master_Gudang_${startDate}_${endDate}`,
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Kelola data lokasi penyimpanan / gudang.
          </p>
        </div>
              ...
              {loading ? (
                ...
                data.map((item) => (
                  <tr key={item.nomor} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">{item.penanggung_jawab || '-'}</td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Link href={`/master/gudang/${item.nomor}?mode=view`} title="View" className="text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
                          <Search className="h-4 w-4" />
                        </Link>
                        <Link href={`/master/gudang/${item.nomor}?mode=edit`} title="Edit" className="text-slate-400 hover:text-amber-600 dark:hover:text-amber-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
