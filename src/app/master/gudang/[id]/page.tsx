...
import { Save, ArrowLeft, Home, AlertCircle, FileEdit, Trash2, X } from "lucide-react";
...
export default function GudangDetail() {
  ...
  const fetchGudang = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setError(data.error || "Gudang tidak ditemukan");
      }
    } catch (err: any) {
  ...
  const handleToggleStatus = async () => {
    if (!confirm(`Apakah Anda yakin ingin ${formData.status_aktif === 1 ? 'menonaktifkan' : 'mengaktifkan'} gudang ini?`)) return;
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
             ...
             <span className="text-sm font-medium text-slate-900 dark:text-white">Detail Gudang</span>
          </div>
          ...
          {!isEdit ? (
            ...
            <>
              <button 
                onClick={() => {
                  setIsEdit(false);
                  fetchGudang(); // fetch again to reset data
                }}
