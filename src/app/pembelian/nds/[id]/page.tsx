...
export default function NDSDetail() {
  const { id } = useParams();
  const [data, setData] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`/api/pembelian/nota-kredit/${id}`)
      .then(r => r.json())
  ...
  const handleApprove = async () => {
    if (!confirm("Setujui NDS ini?")) return;
    const res = await fetch(`/api/pembelian/nota-kredit/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'approve' }) });
    ...
    <div className="max-w-3xl mx-auto space-y-6 pb-12">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-slate-500 mt-0.5">Nota Debet Supplier • {fmtDate(data.tanggal)}</p>
        </div>
        {!data.status_disetujui && (
          <button onClick={handleApprove} className="inline-flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition-colors">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6 text-sm">
          <div><p className="text-xs text-slate-500 mb-1">Kode</p><p className="font-mono font-bold text-purple-600">{data.kode}</p></div>
          <div><p className="text-xs text-slate-500 mb-1">Tanggal</p><p className="font-medium">{fmtDate(data.tanggal)}</p></div>
          <div><p className="text-xs text-slate-500 mb-1">Supplier</p><p className="font-medium">{data.supplier}</p></div>
          <div><p className="text-xs text-slate-500 mb-1">Account Tujuan</p><p className="font-medium">{data.account_tujuan || '-'}</p></div>
          <div><p className="text-xs text-slate-500 mb-1">Kode Nota Beli</p><p className="font-medium">{data.kode_nota_beli || '-'}</p></div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-5">
        <div className="max-w-xs ml-auto space-y-2 text-sm">
          ...
          <div className="flex justify-between border-t pt-2 font-bold text-base"><span>Total</span><span className="text-purple-600">Rp {fmt(data.total)}</span></div>
