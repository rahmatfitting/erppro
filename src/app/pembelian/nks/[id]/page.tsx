...
export default function NKSDetail() {
  const { id } = useParams();
  ...
  const handleApprove = async () => {
    if (!confirm("Setujui NKS ini?")) return;
    ...
    <div className="max-w-3xl mx-auto space-y-6 pb-12">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-slate-500 mt-0.5">Nota Kredit Supplier • {fmtDate(data.tanggal)}</p>
        </div>
        {!data.status_disetujui && (
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6 text-sm">
          <div><p className="text-xs text-slate-500 mb-1">Kode</p><p className="font-mono font-bold text-blue-600">{data.kode}</p></div>
