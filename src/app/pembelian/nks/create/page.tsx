...
import { Save, ArrowLeft, CreditCard, AlertCircle, Search } from "lucide-react";
...
export default function NKSCreate() {
  ...
  const recalc = (data: any) => {
    const grandTotal = parseFloat(data.subtotal || 0) + parseFloat(data.ppnNominal || 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (d.success) router.push(`/pembelian/nks/${d.data.nomor}`);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      <div className="flex items-center gap-3">
        <Link href="/pembelian/nks" className="p-2 -ml-2 rounded-full hover:bg-slate-100 text-slate-500">
          <ArrowLeft className="h-5 w-5" />
        </Link>
        <div className="flex items-center gap-2 flex-1">
          <CreditCard className="h-6 w-6 text-blue-600" />
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">Buat Nota Kredit Supplier (NKS)</h1>
        </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Kode auto - read only placeholder */}
          <div>
          ...
          <div>
            ...
            <input type="date" value={formData.tanggal} onChange={e => setField('tanggal', e.target.value)}
              className="w-full text-sm px-3 py-2 border border-slate-300 dark:border-slate-700 rounded-lg dark:bg-slate-950 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" required />
          </div>
          ...
          <div>
            ...
            <select value={formData.valuta} onChange={e => setField('valuta', e.target.value)}
              className="w-full text-sm px-3 py-2 border border-slate-300 dark:border-slate-700 rounded-lg dark:bg-slate-950 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
          ...
          <div className="md:col-span-2">
            ...
            <textarea value={formData.keterangan} onChange={e => setField('keterangan', e.target.value)} rows={2}
              className="w-full text-sm px-3 py-2 border border-slate-300 dark:border-slate-700 rounded-lg dark:bg-slate-950 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" />
          </div>
        </div>
        ...
        <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <div className="text-sm px-3 py-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg font-bold text-blue-700 dark:text-blue-300">
