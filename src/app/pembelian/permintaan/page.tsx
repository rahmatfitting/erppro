...
import { Plus, Search, Filter, FileText, MoreHorizontal, FileEdit, Trash2, CalendarDays, Loader2, Printer } from "lucide-react";
import { cn } from "@/lib/utils";
import ReportFilterModal from "@/components/ReportFilterModal";
import { exportToPDF, exportToExcel } from "@/lib/exportUtils";

export default function PermintaanPembelianList() {
  ...
  const fetchData = useCallback(async (pageNum: number, isLoadMore = false) => {
    ...
    try {
      ...
      const response = await fetch(`/api/pembelian/permintaan?${queryParams.toString()}`);
  ...
  const handleAction = async (id: string, action: 'approve' | 'disapprove' | 'delete') => {
    if (!confirm(`Apakah Anda yakin ingin ${action === 'delete' ? 'menghapus' : action === 'approve' ? 'menyetujui' : 'menolak'} data ini?`)) return;
    setIsPending(true);
    try {
  ...
  const handleExport = async (startDate: string, endDate: string, exportType: 'view' | 'pdf' | 'excel' | 'pivot') => {
    try {
      ...
      if (result.success) {
        const exportColumns = [
          { header: "No", key: "_no" },
          { header: "Kode PR", key: "kode" },
          { header: "Tanggal", key: "tanggal", format: (v: any) => new Date(v).toLocaleDateString('id-ID') },
          { header: "Divisi", key: "keterangan", format: (v: any) => v?.match(/\[Divisi:\s*(.*?)\]/) ? v.match(/\[Divisi:\s*(.*?)\]/)[1] : '-' },
          { header: "Jml Item", key: "itemsCount" },
          { header: "Status", key: "status_disetujui", format: (v: any) => v === 1 ? "Disetujui" : "Pending" }
        ];

        const config = {
          title: "Laporan Permintaan Pembelian (PR)",
          subtitle: `Periode: ${startDate} s/d ${endDate}`,
          fileName: `Laporan_PR_${startDate}_${endDate}`,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1 dark:text-slate-400">
            Daftar pengajuan permintaan pembelian barang dari berbagai divisi.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
            ...
            <Link
            href="/pembelian/permintaan/create"
            className="inline-flex items-center justify-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
            >
        ...
        <div className="flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1 w-full space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Kode PR atau Divisi..."
                value={filters.keyword}
                onChange={(e) => setFilters(prev => ({ ...prev, keyword: e.target.value }))}
                className="w-full rounded-md border border-slate-200 bg-slate-50 pl-9 pr-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-slate-800 dark:bg-slate-950 dark:text-white"
              />
            </div>
          </div>
          ...
          <div className="w-full md:w-48 space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-md border border-slate-200 bg-white pl-9 pr-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-slate-800 dark:bg-slate-950 dark:text-white"
              />
            </div>
          </div>
          ...
          <div className="w-full md:w-56 space-y-1.5">
            <label className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Divisi</label>
            <select
              value={filters.divisi}
              onChange={(e) => setFilters(prev => ({ ...prev, divisi: e.target.value }))}
              className="w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-slate-800 dark:bg-slate-950 dark:text-white"
            >
              <option value="">Semua Divisi</option>
              <option value="Produksi">Produksi</option>
              <option value="Gudang Utama">Gudang Utama</option>
              <option value="HRD & General Affair">HRD & General Affair</option>
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 overflow-hidden">
          ...
          <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300 relative min-w-[800px]">
            <thead className="bg-slate-50 text-xs uppercase text-slate-700 dark:bg-slate-950 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold">Divisi Peminta</th>
                <th className="px-6 py-4 font-semibold text-center">Jml Item</th>
              ...
              {data.map((item) => (
                <tr key={item.nomor} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/50 transition-colors group">
                  <td className="px-6 py-4 font-medium text-blue-600 dark:text-blue-400">
                    <Link href="#" className="hover:underline">{item.kode}</Link>
                  </td>
                  <td className="px-6 py-4">
                    {new Date(item.tanggal).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' })}
                  </td>
                  <td className="px-6 py-4 font-medium text-slate-900 dark:text-slate-200">
                    {(item.keterangan || '').match(/\[Divisi:\s*(.*?)\]/) ? (item.keterangan.match(/\[Divisi:\s*(.*?)\]/)[1]) : '-'}
                  </td>
                  ...
                  <td className="px-6 py-4 text-center">
                    <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Link href={`/pembelian/permintaan/${item.nomor}`} title="View" className="text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
                      ...
                      {item.status_aktif === 1 && item.status_disetujui === 1 && (
                        <button
                          onClick={() => handleAction(item.nomor, 'disapprove')}
                          disabled={isPending}
                          title="Batalkan Approval"
                          className="text-emerald-600 hover:text-amber-600 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700 disabled:opacity-50"
                        >
                          {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>}
                        </button>
                      )}
                    </div>
                  </td>
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-slate-500 dark:text-slate-400">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p>Tidak ada data Permintaan Pembelian yang ditemukan.</p>
                    </div>
                  </td>
                </tr>
              )}
        ...
        {hasMore && (
          <div ref={observerTarget} className="p-6 flex justify-center items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
            {loading ? (
              <>
                <div className="h-4 w-4 animate-spin rounded-full border-2 border-blue-600 object-top border-t-transparent dark:border-blue-500"></div>
                Memuat data...
              </>
            ) : (
      ...
      <ReportFilterModal
        ...
        title="Print Laporan Permintaan"
