...
import { Plus, Trash2, Save, Receipt, Loader2, PackageOpen, Search } from "lucide-react";
...
export default function NotaBeliLangsungCreate() {
  ...
  const handleSave = async () => {
    try {
      ...
      const payload = {
        ...
        keterangan: header.keterangan || `Faktur Langsung ${header.noFakturSupplier}`,
      ...
      if (!data.success) {
        throw new Error(data.error || "Gagal menyimpan Nota Beli Langsung");
      }

      router.push(`/pembelian/nota-beli-langsung/${data.data.nomor}`);
    } catch (err: any) {
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
            ...
            Nota Beli Langsung (Direct Purchase)
          </h2>
          <p className="text-sm text-slate-500 mt-1 dark:text-slate-400">
            Faktur pembelian langsung yang memotong stok dan hutang tanpa LPB.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.push('/pembelian/nota-beli-langsung')}
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 transition-all">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">No. Nota (Auto)</label>
