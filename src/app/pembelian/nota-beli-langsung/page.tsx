...
export default function NotaBeliLangsungList() {
  ...
  const fetchData = useCallback(async (pageNum: number, isLoadMore = false) => {
    ...
    try {
      ...
      const response = await fetch(`/api/pembelian/nota-beli-langsung?${queryParams.toString()}`);
      ...
      console.error("Failed to fetch Nota Beli Langsung list:", error);
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = async (startDate: string, endDate: string, exportType: 'view' | 'pdf' | 'excel' | 'pivot') => {
    try {
      ...
      if (result.success) {
        ...
        const config = {
          title: "Laporan Nota Beli Langsung",
          subtitle: `Periode: ${startDate} s/d ${endDate}`,
          fileName: `Laporan_Nota_Langsung_${startDate}_${endDate}`,
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
            <Receipt className="h-6 w-6 text-indigo-600 dark:text-indigo-500" />
            Nota Beli Langsung
          </h2>
          <p className="text-sm text-slate-500 mt-1 dark:text-slate-400">
            Daftar pembelian langsung yang memotong stok dan hutang.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
            ...
            <Link 
              href="/pembelian/nota-beli-langsung/create" 
                ...
                <tr key={item.nomor} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <Link href={`/pembelian/nota-beli-langsung/${item.nomor}`} className="font-medium text-indigo-600 dark:text-indigo-400 hover:underline">{item.kode}</Link>
                  ...
                  <td className="px-6 py-4">
                     <span className={cn(
                        "text-sm",
                        (item.status_aktif === 1 && item.status_disetujui === 0 && (Date.now() - new Date(item.jatuh_tempo).getTime() > 0)) ? "text-red-600 font-medium dark:text-red-400" : ""
                     )}>
                  ...
                  <td className="px-6 py-4 text-center">
                    <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Link href={`/pembelian/nota-beli-langsung/${item.nomor}`} title="View" className="text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
                      ...
                      {item.status_aktif === 1 && item.status_disetujui === 0 && (
                        <>
                          ...
                          <Link href={`/pembelian/nota-beli-langsung/edit/${item.nomor}`} title="Edit" className="text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
                ...
                <tr>
                   <td colSpan={7} className="px-6 py-12 text-center text-slate-500 dark:text-slate-400">
                      <div className="flex flex-col items-center justify-center">
                         ...
                         <p>Tidak ada data Nota Beli Langsung yang ditemukan.</p>
                      </div>
                   </td>
                </tr>
              )}
      ...
      <ReportFilterModal
        ...
        title="Print Laporan Nota Beli Langsung"
