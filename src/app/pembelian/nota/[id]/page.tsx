...
import { Plus, Trash2, Save, Receipt, Loader2, ArrowLeft, FileEdit, X, AlertCircle, Printer, History, PackageOpen } from "lucide-react";
...
import { BrowseBarangPenerimaanModal } from "@/components/BrowseBarangPenerimaanModal";
import { BrowseValutaModal } from "@/components/BrowseValutaModal";

export default function NotaBeliDetail() {
      ...
      if (data.success) {
        ...
        if (item.items && item.items.length > 0) {
            setItems(item.items.map((it: any) => ({
                ...it,
                nomorthbelipenerimaan: it.nomorthbelipenerimaan,
                nomortdbelipenerimaan: it.nomortdbelipenerimaan,
                nomormhbarang: it.nomormhbarang,
                nomormhsatuan: it.nomormhsatuan,
                kodePenerimaan: it.kode_pb || "",
                barang: it.barang || it.nama_barang || "",
                satuan: it.satuan || "Pcs",
                jumlah: it.jumlah || 0,
                harga: it.harga || 0,
                diskonPersen: it.diskon_prosentase || 0
            })));
        } else {
            setItems([{ 
              id: Date.now(), 
              nomorthbelipenerimaan: null,
              nomortdbelipenerimaan: null,
              nomormhbarang: null,
              nomormhsatuan: null,
              kodePenerimaan: "",
              barang: "", 
  ...
  const calculations = useMemo(() => {
    ...
    const total = dpp + ppn;
    const totalIdr = total * (header.kurs || 1);
  ...
  }, [items, header.diskonPersen, header.ppnPersen, header.kurs]);
  ...
  const handleSave = async () => {
    try {
      ...
      const data = await res.json();
      if (!data.success) {
        throw new Error(data.error || "Gagal mengupdate data Tagihan/Nota");
      }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12 mt-4 md:mt-0">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm transition-all duration-300">
        <div>
          <div className="flex items-center gap-2 mb-1">
             <Link href="/pembelian/nota" className="text-sm font-medium text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-300 transition-colors">
               Nota Tagihan Beli
             </Link>
          ...
          {!isEdit ? (
            ...
            <>
              ...
              <button 
                onClick={handleSave}
                disabled={loading}
                className="inline-flex items-center justify-center gap-2 rounded-md bg-rose-600 px-6 py-2 text-sm font-bold text-white shadow-lg hover:bg-rose-700 focus:outline-none focus:ring-2 focus:ring-rose-500 transition-all active:scale-95 disabled:opacity-50"
              >
                {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
                {loading ? "Menyimpan Paksa..." : "Simpan Nota"}
              </button>
            </>
          )}
        </div>
      ...
      {activeTab === "detail" ? (
        <div className="animate-in fade-in duration-500 space-y-6">
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 transition-all hover:shadow-md">
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <div className="space-y-2">
                  ...
                  <div className="px-3 py-2.5 text-sm font-bold text-rose-600 bg-rose-50 dark:bg-rose-900/10 rounded-lg border border-rose-100 font-mono">
                    {header.kode}
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">No. Faktur Vendor / Supplier</label>
                  <input
                    type="text"
                    placeholder="INV-..."
                    ...
                    className={`w-full rounded-lg px-3 py-2.5 text-sm outline-none transition-all ${
                      ...
                      : "border border-rose-200 bg-white dark:bg-slate-950 focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500"
                    }`}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <input
                        ...
                        value={header.valuta}
                        readOnly
                        onClick={() => isEdit && setIsBrowseValutaOpen(true)}
                        placeholder="IDR"
                        className={`w-full h-[42px] rounded-lg px-3 text-sm outline-none transition-all font-bold ${
                          !isEdit 
                          ? "bg-slate-50 dark:bg-slate-950/50 text-slate-700 dark:text-slate-300 border-transparent cursor-default" 
                          : "border border-slate-300 bg-white dark:bg-slate-950 focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 cursor-pointer"
                        }`}
                      />
                      {isEdit && (
                        <button
                    ...
                    <input
                      ...
                      readOnly={!isEdit}
                      className={`w-24 h-[42px] rounded-lg px-3 text-sm outline-none transition-all font-mono font-bold text-right ${
                        !isEdit 
                        ? "bg-slate-50 dark:bg-slate-950/50 text-slate-700 dark:text-slate-300 border-transparent cursor-default" 
                        : "border border-slate-300 bg-white dark:bg-slate-950 focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500"
                      }`}
                    />
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Tgl Nota</label>
                  <input
                    type="date"
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className={`w-full rounded-lg px-3 py-2.5 text-sm outline-none transition-all ${
                      !isEdit 
                      ? "bg-slate-50 dark:bg-slate-950/50 text-slate-700 dark:text-slate-300 border-transparent cursor-default font-semibold text-rose-600" 
                      : "border border-slate-300 bg-white dark:bg-slate-950 focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500"
                    }`}
                  />
                </div>
                <div className="space-y-2 lg:col-span-3">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Keterangan / Catatan</label>
                  <input
                    type="text"
                    placeholder={isEdit ? "Catatan tambahan untuk bagian keuangan" : "-"}
                ...
                <table className="w-full text-left text-xs whitespace-nowrap">
                  <thead className="bg-slate-100/50 dark:bg-slate-950 text-[10px] font-black uppercase text-slate-500 sticky top-0 border-b dark:border-slate-800">
                    <tr>
                      ...
                      <th className="px-6 py-4 w-36">Ref LPB</th>
                      <th className="px-6 py-4 min-w-[200px]">Nama Barang / Deskripsi</th>
                      <th className="px-6 py-4 w-24">Satuan</th>
                      <th className="px-6 py-4 w-24 text-right">Jml</th>
                      <th className="px-6 py-4 w-36 text-right">Hrg Satuan</th>
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {calculations.items.map((item, index) => (
                      <tr key={item.id} className="hover:bg-rose-50/20 dark:hover:bg-rose-500/5 transition-colors">
                        <td className="px-6 py-4 text-center text-slate-400 font-mono">
                          {index + 1}
                        </td>
                        <td className="px-6 py-3">
                          {item.nomorthbelipenerimaan ? (
                            <Link 
                              href={`/pembelian/penerimaan/${item.nomorthbelipenerimaan}`}
                              className="text-[10px] tracking-tight text-rose-600 font-bold hover:underline font-mono uppercase"
                            >
                              {item.kodePenerimaan}
                            </Link>
                          ) : (
                            <span className="text-[10px] tracking-tight text-slate-400 font-mono uppercase">
                              {item.kodePenerimaan || "-"}
                            </span>
                          )}
                        </td>
                        <td className="px-6 py-3 font-semibold text-slate-700 dark:text-slate-300">
                          <input
                            ...
                            value={item.barang}
                            readOnly
                            className="w-full border-none bg-transparent outline-none cursor-default"
                          />
                        </td>
                        <td className="px-6 py-3">
                        ...
                        <td className="px-6 py-3 text-right">
                          <input
                            ...
                            value={item.jumlah}
                            readOnly
                            className="w-full text-right rounded-lg px-3 py-2 outline-none transition-all font-bold bg-transparent text-indigo-600 cursor-default"
                          />
                        </td>
                        <td className="px-6 py-3 text-right">
                          <input
                            ...
                            readOnly={!isEdit}
                            className={`w-full text-right rounded-lg px-3 py-2 outline-none transition-all font-mono font-bold ${
                                !isEdit ? "bg-transparent text-slate-700 dark:text-slate-300" : "bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 focus:ring-1 focus:ring-rose-500"
                            }`}
                          />
                        </td>
                        <td className="px-6 py-3 text-center">
                           <div className="flex items-center justify-center">
                              <input
                                  ...
                                  readOnly={!isEdit}
                                  className={`w-12 rounded-lg px-1 py-2 text-center outline-none transition-all text-red-500 font-bold ${
                                      !isEdit ? "bg-transparent text-slate-400" : "bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700"
                                  }`}
                              />
                           </div>
                        </td>
                        <td className="px-6 py-4 text-right font-black text-slate-900 dark:text-white bg-slate-50/20">
            ...
            <div className="flex justify-end">
              <div className="w-full max-w-md rounded-2xl border-2 border-rose-100 bg-white p-6 shadow-sm dark:border-rose-900/30 dark:bg-slate-900 transition-all hover:shadow-md">
                <h3 className="mb-6 text-xs font-black uppercase tracking-widest text-slate-400 flex items-center justify-between">
                  Rangkuman Tagihan
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                    <div className="flex items-center justify-between">
                      <div className="flex flex-col">
                        <span className="text-[10px] font-bold text-slate-400 uppercase">Disc Global (%)</span>
                        <div className="flex items-center mt-1">
                            <input 
                              type="number" 
                              min="0" max="100" 
                              className={`w-14 h-8 text-sm font-bold text-center rounded-lg outline-none transition-all ${
                                 !isEdit ? "bg-slate-50 text-slate-400 border-transparent" : "bg-white border border-slate-200 focus:border-rose-500"
                              }`}
                  ...
                  <div className="flex justify-between items-center pt-2">
                    ...
                    <span className="text-xs font-black text-slate-600 dark:text-slate-400 font-mono tracking-wider">
                  ...
                  <div className="pt-2">
                    <div className="flex items-center justify-between">
                      <div className="flex flex-col">
                        <span className="text-[10px] font-bold text-slate-400 uppercase">Pajak (PPN %)</span>
                        <div className="flex items-center mt-1">
                            <input 
                              type="number" 
                              min="0" max="100" 
                    ...
                    <div className="flex flex-col items-end text-right">
                      ...
                      {header.valuta !== "IDR" && (
                        <div className="mt-2 text-right">
                          <span className="text-[10px] font-bold text-slate-400 uppercase block leading-none mb-1">Total dalam IDR</span>
                          <span className="text-sm font-bold text-slate-600 dark:text-slate-400 font-mono italic">
                            Rp {new Intl.NumberFormat('id-ID').format(calculations.totalIdr)}
                          </span>
                        </div>
                      )}
                    </div>
        ...
        <HistoryLogTab menu="Nota Pembelian" nomor_transaksi={header.nomor} />
      )}
      ...
      <PrintModal
        ...
        data={{
          title: "Nota / Invoice Pembelian",
          kode: header.kode,
          tanggal: header.tanggal,
          keterangan: header.keterangan,
          extraHeaders: [
            { label: "Supplier", value: header.supplier || "-" },
            { label: "No. Faktur Vendor", value: header.noFakturSupplier || "-" },
            { label: "Jatuh Tempo", value: header.jatuhTempo || "-" },
          ],
          columns: [
            { header: "No", key: "_no", width: 8, align: "center" },
            { header: "Ref LPB", key: "kodePenerimaan", width: 30 },
          ...
          footerRows: [
            { label: "Subtotal Tagihan", value: calculations.subtotal.toLocaleString('id-ID') },
            { label: "Disc Global", value: `-${calculations.diskon.toLocaleString('id-ID')}` },
            { label: "DPP Pajak", value: calculations.dpp.toLocaleString('id-ID') },
            { label: "PPN Pajak", value: calculations.ppn.toLocaleString('id-ID') },
            { label: "TOTAL AKHIR", value: `${header.valuta} ${calculations.total.toLocaleString('id-ID')}` },
          ]
        }}
      />
      {/* Modals */}
      ...
      <BrowsePenerimaanModal
        ...
        onSelect={(pb: any) => {
            setHeader({ ...header, supplier: pb.supplier, nomormhsupplier: pb.nomormhsupplier });
      ...
      <BrowseBarangPenerimaanModal
        isOpen={isBrowseBarangOpen}
        onClose={() => setIsBrowseBarangOpen(false)}
        onSelect={(item: any) => {
            if (items.find(i => i.nomortdbelipenerimaan === item.nomortdbelipenerimaan)) {
                setError("Barang dari LPB ini sudah ada di daftar");
            ...
            setItems([
                ...
                { 
                    ...
                    nomormhbarang: item.nomormhbarang,
                    nomormhsatuan: item.nomormhsatuan,
                    kodePenerimaan: currentPB.kode,
                    kode_barang: item.kode_barang,
                    barang: item.barang || item.nama_barang, 
                    satuan: item.satuan, 
                    jumlah: item.jumlahDiterima || item.jumlah, 
                    harga: item.harga || 0, 
                    diskonPersen: 0
                },
            ]);
