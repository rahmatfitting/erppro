...
import { Plus, Search, Filter, Truck, MoreHorizontal, FileEdit, Trash2, CalendarDays, Loader2, X } from "lucide-react";
...
export default function PenerimaanBarangList() {
  ...
  const fetchData = useCallback(async (pageNum: number, isLoadMore = false) => {
    ...
    try {
      ...
      const response = await fetch(`/api/pembelian/penerimaan?${queryParams.toString()}`);
      ...
      console.error("Failed to fetch Penerimaan list:", error);
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = async (startDate: string, endDate: string, exportType: 'view' | 'pdf' | 'excel' | 'pivot') => {
    try {
      ...
      setLoading(true);
      // Fetch data for the specified range without limit
      const queryParams = new URLSearchParams({
        startDate,
        endDate,
        limit: "999999", // Large limit for export
        keyword: filters.keyword,
        supplier: filters.supplier,
        gudang: filters.gudang
      });
      ...
      if (result.success) {
        ...
        const config = {
          ...
          fileName: `Laporan_Penerimaan_${startDate}_${endDate}`,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1 dark:text-slate-400">
            Daftar penerimaan barang masuk dari supplier.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
            ...
            <Link 
            href="/pembelian/penerimaan/create" 
            className="inline-flex items-center justify-center gap-2 rounded-md bg-emerald-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-colors"
            >
        ...
        <div className="flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1 w-full space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Kode PB, PO, atau Supplier..."
                value={filters.keyword}
                onChange={(e) => setFilters(prev => ({ ...prev, keyword: e.target.value }))}
                className="w-full rounded-md border border-slate-200 bg-slate-50 pl-9 pr-3 py-2 text-sm outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 dark:border-slate-800 dark:bg-slate-950 dark:text-white"
              />
            </div>
          </div>
          ...
          <div className="w-full md:w-48 space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-md border border-slate-200 bg-white pl-9 pr-3 py-2 text-sm outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 dark:border-slate-800 dark:bg-slate-950 dark:text-white"
              />
            </div>
          </div>
          ...
          <div className="w-full md:w-56 space-y-1.5">
             ...
             <div className="relative">
                <input
                  ...
                  className="w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm outline-none cursor-pointer focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 dark:border-slate-800 dark:bg-slate-950 dark:text-white"
                />
                {filters.supplier && (
                  <button 
          ...
          <div className="w-full md:w-56 space-y-1.5">
             <label className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Gudang</label>
             <div className="relative">
                <input
                  type="text"
                  readOnly
                ...
                {filters.gudang && (
                  <button 
                    onClick={() => setFilters(prev => ({ ...prev, gudang: "" }))}
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 rounded-full hover:bg-slate-100"
                  >
                    <X className="h-3 w-3" />
                  </button>
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300 relative min-w-[800px]">
            <thead className="bg-slate-50 text-xs uppercase text-slate-700 dark:bg-slate-950 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
              <tr>
                <th className="px-6 py-4 font-semibold">Kode Penerimaan</th>
                ...
                <tr key={item.nomor} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-center">
                    <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Link href={`/pembelian/penerimaan/${item.nomor}`} title="View" className="text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
                      ...
                      {item.status_aktif === 1 && item.status_disetujui === 0 && (
                        <>
                          ...
                          <button 
                            onClick={() => handleAction(item.nomor, 'reject')} 
                            disabled={isPending}
                            title="Disapprove" 
        ...
        {hasMore && (
           <div ref={observerTarget} className="p-6 flex justify-center items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
             {loading ? (
                <>
                  <div className="h-4 w-4 animate-spin rounded-full border-2 border-emerald-600 object-top border-t-transparent dark:border-emerald-500"></div>
                  Memuat data...
                </>
             ) : (
      ...
      <BrowseGudangModal
        ...
        onSelect={(gudang) => {
          setFilters(prev => ({ ...prev, gudang: gudang.nama }));
      ...
      <ReportFilterModal
        ...
        title="Print Laporan Penerimaan Barang"
