...
import { Plus, Search, Receipt, Loader2, X, Printer, CalendarDays, Trash2 } from "lucide-react";
import { cn } from "@/lib/utils";
import ReportFilterModal from "@/components/ReportFilterModal";
import { exportToPDF, exportToExcel } from "@/lib/exportUtils";
// Make sure BrowseCustomerModal exists in components or adapt it
import { BrowseCustomerModal } from "@/components/BrowseCustomerModal";

export default function NotaJualEmasList() {
  ...
  const fetchData = useCallback(async (pageNum: number, isLoadMore = false) => {
    ...
    try {
      ...
      const response = await fetch(`/api/penjualan/nota-emas?${queryParams.toString()}`);
  ...
  const handleExport = async (startDate: string, endDate: string, exportType: 'view' | 'pdf' | 'excel' | 'pivot') => {
    try {
      ...
      if (result.success) {
         // Implement Export function parameters here...
      }
      setIsReportModalOpen(false);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
            <Receipt className="h-6 w-6 text-amber-600 dark:text-amber-500" />
            Nota Jual Emas
          </h2>
          <p className="text-sm text-slate-500 mt-1 dark:text-slate-400">
            Daftar transaksi penjualan komoditas emas langsung ke Kustomer.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
            ...
            <Link 
              href="/penjualan/nota-emas/create" 
              className="inline-flex items-center justify-center gap-2 rounded-md bg-amber-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-amber-500 transition-colors"
            >
        ...
        <div className="flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1 w-full space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="No. Nota Jual Emas..."
                value={filters.keyword}
                onChange={(e) => setFilters(prev => ({ ...prev, keyword: e.target.value }))}
                className="w-full rounded-md border border-slate-200 bg-slate-50 pl-9 pr-3 py-2 text-sm outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 dark:border-slate-800 dark:bg-slate-950 dark:text-white"
              />
            </div>
          </div>
          ...
          <div className="w-full md:w-32 space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-md border border-slate-200 bg-white pl-9 pr-3 py-2 text-sm outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 dark:border-slate-800 dark:bg-slate-950 dark:text-white"
              />
            </div>
          </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 overflow-hidden">
          ...
          <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300 relative min-w-[900px]">
            <thead className="bg-slate-50 text-xs uppercase text-slate-700 dark:bg-slate-950 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold">Tgl Nota / Jatuh Tempo</th>
                ...
                <tr key={item.nomor} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <Link href={`/penjualan/nota-emas/${item.nomor}`} className="font-medium text-amber-600 dark:text-amber-400 hover:underline">{item.kode}</Link>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                     <div className="flex flex-col gap-1 text-xs">
                        <span>{new Date(item.tanggal).toLocaleDateString('id-ID')}</span>
                        <span className={cn("text-[11px]", (item.status_aktif===1 && item.status_disetujui===0 && (Date.now() - new Date(item.jatuh_tempo).getTime() > 0)) ? "text-rose-600 font-bold" : "text-slate-400")}>
                           JT: {new Date(item.jatuh_tempo).toLocaleDateString('id-ID')}
                        </span>
                     </div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-center">
                    <div className="flex items-center gap-2">
                      <Link href={`/penjualan/nota-emas/${item.nomor}`} title="Print/View PDF" className="text-slate-400 hover:text-amber-600 dark:hover:text-amber-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
                        <Printer className="h-4 w-4" />
                      </Link>
                      ...
                      {item.status_aktif === 1 && item.status_disetujui === 0 && (
                        <>
                          <button 
                            ...
                            disabled={isPending}
                            title="Approve (Kurangi Stok & Tambah Piutang)" 
                      ...
                      {item.status_aktif === 1 && item.status_disetujui === 1 && (
                          <button 
                            ...
                            disabled={isPending}
                            title="Batal Approve (Kembalikan Stok & Hapus Piutang)" 
                            className="text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700 disabled:opacity-50"
                          >
                ...
                <tr>
                   <td colSpan={6} className="px-6 py-12 text-center text-slate-500 dark:text-slate-400">
                      <div className="flex flex-col items-center justify-center">
                         ...
                         <p>Tidak ada data Nota Jual Emas yang ditemukan.</p>
                      </div>
                   </td>
                </tr>
              )}
        ...
        {hasMore && (
           <div ref={observerTarget} className="p-6 flex justify-center items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
             {loading ? (
                <>
                  <div className="h-4 w-4 animate-spin rounded-full border-2 border-amber-600 object-top border-t-transparent dark:border-amber-500"></div>
                  Memuat data...
                </>
             ) : (
      ...
      <ReportFilterModal
        ...
        title="Print Laporan Penjualan Emas"
