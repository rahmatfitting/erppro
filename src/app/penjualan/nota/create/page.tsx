...
import { Save, ArrowLeft, Receipt, AlertCircle, Plus, Trash2, Search } from "lucide-react";
import { useRouter } from "next/navigation";
import Link from "next/link";
import { cn } from "@/lib/utils";

export default function CreateNotaJual() {
  ...
  const [sjList, setSjList] = useState<any[]>([]); // Surat Jalan list

  useEffect(() => {
    // Fetch master data
    const fetchMasters = async () => {
      try {
        const [resCust, resSales, resBarang, resSJ] = await Promise.all([
          fetch('/api/master/customer'),
          fetch('/api/master/sales'),
          fetch('/api/master/barang'),
          fetch('/api/penjualan/surat-jalan') // fetch Surat Jalan to pick from
        ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/penjualan/nota", {
        method: "POST",
      ...
      if (data.success) {
        router.push(`/penjualan/nota/${data.data.kode}`);
      } else {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="p-6 md:p-8 space-y-6 border-b border-slate-200 dark:border-slate-800">
             <div className="flex justify-between items-center mb-4">
                ...
                {/* Reference SJ Selection */}
                <div className="relative">
                   <select name="kode_surat_jalan" value={header.kode_surat_jalan} onChange={handleHeaderChange} className="pl-10 pr-4 py-2 text-sm border-2 border-indigo-200 bg-indigo-50 text-indigo-700 rounded-lg outline-none focus:border-indigo-500 font-medium">
                      <option value="">-- Tagih dari Surat Jalan (SJ) --</option>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Tanggal Invoice <span className="text-red-500">*</span></label>
                ...
                <div className="space-y-2">
                  ...
                  <select name="sales" value={header.sales || ""} onChange={handleHeaderChange} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm dark:bg-slate-950 dark:border-slate-700 dark:text-white focus:border-indigo-500 outline-none">
                ...
                <div className="space-y-2 flex gap-4">
                   <div className="flex-1">
                     ...
                     <input type="text" name="valuta" required value={header.valuta || "IDR"} onChange={handleHeaderChange} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm dark:bg-slate-950 dark:border-slate-700 dark:text-white focus:border-indigo-500 outline-none"/>
                   </div>
                   <div className="flex-1">
                     <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Kurs <span className="text-red-500">*</span></label>
                     <input type="number" name="kurs" required min="1" value={header.kurs || 1} onChange={handleHeaderChange} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm dark:bg-slate-950 dark:border-slate-700 dark:text-white focus:border-indigo-500 outline-none"/>
                   </div>
                </div>
                
                <div className="space-y-2 lg:col-span-2">
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Keterangan / Notes Tagihan</label>
                  <textarea name="keterangan" rows={1} value={header.keterangan || ""} onChange={handleHeaderChange} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm dark:bg-slate-950 dark:border-slate-700 dark:text-white focus:border-indigo-500 outline-none" placeholder="Catatan invoice..."/>
                </div>
             </div>
          </div>
                   ...
                   <tbody className="divide-y divide-slate-200 dark:divide-slate-800 bg-white dark:bg-slate-950">
                      {items.map((item, index) => (
                         <tr key={index} className="group hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                            ...
                            <td className="p-2">
                               <input type="number" name="jumlah" min="1" step="0.01" value={item.jumlah || 0} onChange={(e) => handleItemChange(index, e)} className="w-full text-sm p-2 rounded border border-indigo-300 focus:border-indigo-500 bg-indigo-50/50 outline-none text-center" required/>
                            </td>
                            <td className="p-2">
                               <input type="number" name="harga" min="0" step="0.01" value={item.harga || 0} onChange={(e) => handleItemChange(index, e)} className="w-full text-sm p-2 rounded border border-slate-300 dark:border-slate-700 bg-transparent outline-none"/>
                            </td>
                            <td className="p-2">
                               <input type="number" name="diskon_prosentase" min="0" max="100" step="0.01" value={item.diskon_prosentase || 0} onChange={(e) => handleItemChange(index, e)} className="w-full text-sm p-2 rounded border border-slate-300 dark:border-slate-700 bg-transparent outline-none"/>
                            </td>
                            <td className="p-2">
          ...
          {/* Totals Section */}
          <div className="p-6 md:p-8 border-b border-slate-200 dark:border-slate-800 flex justify-end bg-white dark:bg-slate-900">
             <div className="w-full md:w-1/2 lg:w-1/3 space-y-3">
                ...
                <div className="flex justify-between items-center pt-3 border-t-2 border-slate-300 dark:border-slate-700">
                   <span className="text-base font-bold text-slate-900 dark:text-white">Grand Total Tagihan</span>
