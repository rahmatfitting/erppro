...
import { Plus, Search, Receipt, Loader2, Edit, AlertCircle, Trash2, CheckCircle, Printer } from "lucide-react";
import Link from "next/link";
import { cn } from "@/lib/utils";
import ReportFilterModal from "@/components/ReportFilterModal";
import { exportToPDF, exportToExcel } from "@/lib/exportUtils";

export default function NotaJualPage() {
  ...
  const fetchData = async (keyword = "") => {
    setLoading(true);
    try {
      const res = await fetch(`/api/penjualan/nota?keyword=${keyword}`);
      const json = await res.json();
      if (json.success) setData(json.data);
      else setError(json.error);
    } catch (err) {
      setError("Gagal mengambil data Nota Jual");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExport = async (startDate: string, endDate: string, exportType: 'view' | 'pdf' | 'excel' | 'pivot') => {
    try {
      ...
      const response = await fetch(`/api/penjualan/nota?${queryParams.toString()}`);
      const result = await response.json();

      if (result.success) {
        const exportColumns = [
          { header: "No", key: "_no" },
          { header: "No. Invoice", key: "kode" },
          { header: "Ref SJ/SO", key: "kode_surat_jalan", format: (v: any, row: any) => v || row.kode_order_jual || "-" },
          { header: "Tanggal", key: "tanggal", format: (v: any) => new Date(v).toLocaleDateString('id-ID') },
          { header: "Customer", key: "customer" },
          { header: "Total Tagihan", key: "total", format: (v: any, row: any) => (row.valuta || "IDR") + " " + new Intl.NumberFormat('id-ID').format(v) },
          { header: "Status", key: "status_disetujui", format: (v: any, row: any) => row.status_aktif === 0 ? "Dibatalkan" : v === 1 ? "Disetujui" : "Pending" }
        ];

        const config = {
          title: "Laporan Nota Penjualan / Invoice",
          subtitle: `Periode: ${startDate} s/d ${endDate}`,
          fileName: `Invoice_Penjualan_${startDate}_${endDate}`,
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Kelola data dokumen tagihan (Invoice) penjualan.
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <form onSubmit={handleSearch} className="flex gap-2">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              type="text"
              placeholder="Cari no Invoice, SJ, atau Kustomer..."
              value={searchQuery}
          ...
          <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300">
            <thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-200 uppercase text-xs font-semibold border-b border-slate-200 dark:border-slate-800">
              <tr>
                ...
                <th className="px-6 py-4">Ref SJ/SO</th>
                  ...
                  return (
                    <tr key={item.kode} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-indigo-600 dark:text-indigo-400 font-medium">
                         {item.kode_surat_jalan || item.kode_order_jual || '-'}
                      </td>
                      ...
                      <td className="px-6 py-4 text-right font-medium text-slate-900 dark:text-white">
                         {item.valuta} {new Intl.NumberFormat('id-ID').format(item.total)}
                      </td>
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Link href={`/penjualan/nota/${item.kode}?mode=view`} title="View Detail" className="text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
                             <Search className="h-4 w-4" />
                          </Link>

                           {!isCanceled && !isApproved && (
                              <>
                               <Link href={`/penjualan/nota/${item.kode}?mode=edit`} title="Edit" className="text-slate-400 hover:text-amber-600 dark:hover:text-amber-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
