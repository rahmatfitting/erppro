...
import { Save, ArrowLeft, FileText, AlertCircle, Plus, Trash2, FileEdit, X, Search, Printer } from "lucide-react";
...
export default function SuratJalanDetail() {
  ...
  const fetchData = async () => {
    setFetching(true);
    try {
      const [resCust, resSales, resGudang, resBarang, resDO, resDetail] = await Promise.all([
        fetch('/api/master/customer'),
        fetch('/api/master/sales'),
        fetch('/api/master/gudang'),
        fetch('/api/master/barang'),
        fetch('/api/penjualan/delivery'),
        fetch(`/api/penjualan/surat-jalan/${id}`)
      ]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/penjualan/surat-jalan/${id}`, {
        method: "PUT",
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-12">
      <div className="flex justify-between items-start">
        <div>
          <div className="flex items-center gap-2 mb-1">
             ...
             <span className="text-sm font-medium text-slate-900 dark:text-white">Detail SJ</span>
          </div>
        ...
        <div className="flex gap-2">
           {!isEdit ? (
             <>
               <Link href="/penjualan/surat-jalan" className="inline-flex items-center gap-2 rounded-md bg-white border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50">
      ...
      {activeTab === "detail" || isEdit ? (
        ...
        <form onSubmit={handleSave}>
          <div className="p-6 md:p-8 space-y-6 border-b border-slate-200 dark:border-slate-800">
             <div className="flex justify-between items-center mb-4">
               ...
               {isEdit && (
                 <select name="kode_delivery_order" value={header.kode_delivery_order} onChange={handleHeaderChange} className="pl-4 pr-10 py-2 text-sm border-2 border-indigo-200 bg-indigo-50 text-indigo-700 rounded-lg outline-none focus:border-indigo-500 font-medium">
                    <option value="">-- Ubah Tarik DO --</option>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <input type="text" name="nomor_kendaraan" readOnly={!isEdit} value={header.nomor_kendaraan} onChange={handleHeaderChange} className={cn("w-full rounded-md px-3 py-2 text-sm outline-none", !isEdit ? "bg-slate-50 border border-slate-200 text-slate-500" : "bg-white border border-slate-300 focus:border-indigo-500 focus:ring-1")}/>
                </div>

                <div className="space-y-2">
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Pengemudi / Sopir</label>
                  <input type="text" name="sopir" readOnly={!isEdit} value={header.sopir} onChange={handleHeaderChange} className={cn("w-full rounded-md px-3 py-2 text-sm outline-none", !isEdit ? "bg-slate-50 border border-slate-200 text-slate-500" : "bg-white border border-slate-300 focus:border-indigo-500 focus:ring-1")}/>
                </div>
                ...
                <div className="space-y-2 lg:row-span-2">
                  ...
                  <textarea name="keterangan" rows={4} readOnly={!isEdit} value={header.keterangan} onChange={handleHeaderChange} className={cn("w-full rounded-md px-3 py-2 text-sm outline-none resize-none", !isEdit ? "bg-slate-50 border border-slate-200 text-slate-500" : "bg-white border border-slate-300 focus:border-indigo-500")}/>
                </div>
             </div>
          </div>
                   ...
                   <tbody className="divide-y divide-slate-200 bg-white">
                      {items.map((item, index) => (
                         <tr key={index} className="group hover:bg-slate-50 transition-colors">
                            ...
                            <td className="p-2">
                               <input type="number" name="jumlah" min="1" step="0.01" readOnly={!isEdit} value={item.jumlah} onChange={(e) => handleItemChange(index, e)} className={cn("w-full text-sm p-3 rounded outline-none text-center font-semibold", isEdit ? "border border-indigo-300 bg-indigo-50/50 focus:border-indigo-500 text-indigo-700" : "border-none bg-transparent text-indigo-600")} required/>
                            </td>
                            {isEdit && (
        ...
        <HistoryLogTab menu="Surat Jalan" nomor_transaksi={header.nomor} />
      )}
      ...
      <PrintModal
        ...
        data={{
          ...
          extraHeaders: [
            ...
            { label: "No. Kendaraan", value: header.nomor_kendaraan || "-" },
            { label: "Sopir", value: header.sopir || "-" },
          ],
          columns: [
            ...
            { header: "Jumlah Fisik", key: "jumlah", width: 25, align: "right" },
          ],
          rows: items,
        }}
      />
