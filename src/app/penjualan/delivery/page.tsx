...
import { Plus, Search, Truck, Loader2, Edit, AlertCircle, Trash2, CheckCircle } from "lucide-react";
...
export default function DeliveryOrderPage() {
  ...
  const fetchData = async (keyword = "") => {
    setLoading(true);
    try {
      const res = await fetch(`/api/penjualan/delivery?keyword=${keyword}`);
      const json = await res.json();
      if (json.success) setData(json.data);
      else setError(json.error);
    } catch (err) {
      setError("Gagal mengambil data Delivery Order");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Kelola data Surat Perintah Pengiriman Barang.
          </p>
        </div>
        <Link 
          href="/penjualan/delivery/create"
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <form onSubmit={handleSearch} className="flex gap-2">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              type="text"
              placeholder="Cari no DO, SO atau nama customer..."
              value={searchQuery}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300">
            <thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-200 uppercase text-xs font-semibold border-b border-slate-200 dark:border-slate-800">
              <tr>
                <th className="px-6 py-4">No. DO</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={8} className="px-6 py-8 text-center text-slate-500 dark:text-slate-400">
                    Tidak ada data Delivery Order yang ditemukan.
                  </td>
                </tr>
              ) : (
                data.map((item) => {
                  ...
                  return (
                    <tr key={item.kode} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                      ...
                      <td className="px-6 py-4">{item.gudang}</td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Link href={`/penjualan/delivery/${item.kode}?mode=view`} title="View Detail" className="text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
                             <Search className="h-4 w-4" />
                          </Link>

                          {!isCanceled && !isApproved && (
                             <>
                              <Link href={`/penjualan/delivery/${item.kode}?mode=edit`} title="Edit" className="text-slate-400 hover:text-amber-600 dark:hover:text-amber-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
                                <Edit className="h-4 w-4" />
                              </Link>
                              <button onClick={() => handleAction(item.kode, 'approve')} title="Approve" className="text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
                                 <CheckCircle className="h-4 w-4" />
                              </button>
                              <button onClick={() => handleAction(item.kode, 'delete')} title="Batalkan" className="text-slate-400 hover:text-red-600 dark:hover:text-red-400 bg-white shadow-sm border border-slate-200 rounded p-1.5 dark:bg-slate-900 dark:border-slate-700">
