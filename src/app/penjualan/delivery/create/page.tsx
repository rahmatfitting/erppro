...
import { Save, ArrowLeft, Truck, AlertCircle, Plus, Trash2, Search } from "lucide-react";
...
export default function CreateDeliveryOrder() {
  ...
  useEffect(() => {
    ...
    const fetchMasters = async () => {
      try {
        const [resCust, resSales, resGudang, resBarang, resSO] = await Promise.all([
          fetch('/api/master/customer'),
          fetch('/api/master/sales'),
          fetch('/api/master/gudang'),
          fetch('/api/master/barang'),
          fetch('/api/penjualan/order') // fetch un-completed SO based on logic, but API provides all for now
        ]);
       ...
       if (json.success && json.data) {
          ...
          if (so.items && so.items.length > 0) {
             setItems(so.items.map((i:any) => ({
                 ...
                 satuan: i.satuan,
                 jumlah: i.jumlah, // DO might adjust qty, but we default to SO qty
                 harga: i.harga,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/penjualan/delivery", {
        method: "POST",
      ...
      if (data.success) {
        router.push(`/penjualan/delivery/${data.data.kode}`);
      } else {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="p-6 md:p-8 space-y-6 border-b border-slate-200 dark:border-slate-800">
             <div className="flex justify-between items-center mb-4">
                ...
                {/* Reference SO Selection */}
                <div className="relative">
                   <select name="kode_order_jual" value={header.kode_order_jual} onChange={handleHeaderChange} className="pl-10 pr-4 py-2 text-sm border-2 border-indigo-200 bg-indigo-50 text-indigo-700 rounded-lg outline-none focus:border-indigo-500 font-medium">
                      <option value="">-- Tarik dari Sales Order (SO) --</option>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <select name="gudang" required value={header.gudang} onChange={handleHeaderChange} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm dark:bg-slate-950 dark:border-slate-700 dark:text-white focus:border-indigo-500 focus:ring-1 outline-none">
                ...
                <div className="space-y-2">
                  ...
                  <select name="customer" required value={header.customer} onChange={handleHeaderChange} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm dark:bg-slate-950 dark:border-slate-700 dark:text-white focus:border-indigo-500 focus:ring-1 outline-none">
                ...
                <div className="space-y-2">
                  ...
                  <select name="sales" value={header.sales} onChange={handleHeaderChange} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm dark:bg-slate-950 dark:border-slate-700 dark:text-white focus:border-indigo-500 focus:ring-1 outline-none">
                ...
                <div className="space-y-2 lg:col-span-3">
                  ...
                  <textarea name="keterangan" rows={2} value={header.keterangan} onChange={handleHeaderChange} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm dark:bg-slate-950 dark:border-slate-700 dark:text-white focus:border-indigo-500 outline-none resize-none" placeholder="Catatan pengiriman..."/>
                </div>
             </div>
          </div>
                         ...
                         <tr key={index} className="group hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                            ...
                            <td className="p-2">
                               ...
                               {(!barangs.some(b => b.kode === item.kode_barang) && item.kode_barang !== "") && (
                                 <input type="text" name="nama_barang" placeholder="Nama Barang" value={item.nama_barang} onChange={(e) => handleItemChange(index, e)} className="w-full mt-2 text-sm p-2 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 outline-none" required/>
                               )}
                            </td>
                            <td className="p-2">
                            ...
                            <td className="p-2">
                               <input type="number" name="jumlah" min="1" step="0.01" value={item.jumlah} onChange={(e) => handleItemChange(index, e)} className="w-full text-sm p-2 rounded border border-slate-300 border-indigo-300 dark:border-indigo-800 focus:border-indigo-500 dark:bg-indigo-900/20 bg-indigo-50/50 outline-none" required/>
                            </td>
                            <td className="p-2">
