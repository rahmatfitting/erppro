...
export async function GET(request: Request) {
  try {
    ...
    if (keyword) {
      query += ` AND (h.kode LIKE ? OR h.customer LIKE ? OR h.kode_delivery_order LIKE ? OR h.nomor_kendaraan LIKE ?)`;
    ...
    console.error("GET Surat Jalan Error:", error);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { 
      tanggal, customer, sales, gudang, kode_order_jual, kode_delivery_order, 
      nomor_kendaraan, sopir, valuta, kurs, keterangan, 
    ...
    const prefix = `SJ-${year}${month}-`;

    const [rows]: any = await connection.execute(
      `SELECT kode FROM thsuratjalan WHERE kode LIKE ? ORDER BY kode DESC LIMIT 1 FOR UPDATE`,
      [`${prefix}%`]
    );
    ...
    const [headerResult]: any = await connection.execute(
      `INSERT INTO thsuratjalan 
        (kode, tanggal, gudang, nomor_kendaraan, sopir, customer, sales, kode_order_jual, kode_delivery_order, valuta, kurs, keterangan, 
         subtotal, diskon_nominal, dpp, ppn_nominal, total, total_idr, dibuat_oleh) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        generatedKode, tanggal, gudang, nomor_kendaraan || '', sopir || '', customer, sales || '', 
        kode_order_jual || '', kode_delivery_order || '', valuta || 'IDR', kurs || 1, keterangan || '',
    ...
    await addLogHistory("Surat Jalan", headerId, "CREATE", user || "Admin", `Membuat Surat Jalan baru ${generatedKode}`);
    return NextResponse.json({ success: true, message: "Surat Jalan berhasil disimpan", data: { id: headerId, kode: generatedKode } });

  } catch (error: any) {
    await connection.rollback();
    console.error("POST Surat Jalan Error:", error);
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  } finally {
    connection.release();
  }
...
export async function PATCH(request: Request) {
  try {
    ...
    if (action === 'approve') {
       query = `UPDATE thsuratjalan SET status_disetujui = 1, disetujui_oleh = 'Admin', disetujui_pada = NOW() WHERE kode = ?`;
       params.push(id);
    } else if (action === 'reject' || action === 'delete') {
       query = `UPDATE thsuratjalan SET status_aktif = 0, dibatalkan_oleh = 'Admin', dibatalkan_pada = NOW() WHERE kode = ?`;
       params.push(id);
    } else {
    ...
    const headerData: any = await executeQuery(`SELECT nomor FROM thsuratjalan WHERE kode = ?`, [id]);
    if (headerData.length > 0) {
      const headerId = headerData[0].nomor;
      const logAction = action === 'approve' ? 'APPROVE' : 'DELETE';
      const logDesc = action === 'approve' ? `Menyetujui Surat Jalan ${id}` : `Membatalkan Surat Jalan ${id}`;
      await addLogHistory("Surat Jalan", headerId, logAction, user || "Admin", logDesc);
    }

    return NextResponse.json({ success: true, message: `Berhasil ${action} data` });
  } catch (error: any) {
    console.error("PATCH Surat Jalan Error:", error);
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}
