...
export async function GET(request: Request) {
  try {
    ...
    if (type === 'kartu') {
      if (!nomormhcustomer || !startDate) {
        return NextResponse.json({ success: false, error: 'Customer dan Tanggal Awal wajib diisi untuk Kartu Piutang' }, { status: 400 });
      }

      // 1. Get Opening Balance
      const [opening]: any = await executeQuery(`
        SELECT COALESCE(SUM(total_idr), 0) as balance 
        FROM rhlaporanpiutang 
        WHERE nomormhperusahaan = ? AND nomormhcustomer = ? AND DATE(tanggal) < ?
          AND nomormhcabang = ?
      `, [active_perusahaan, nomormhcustomer, startDate, active_cabang]);
      ...
      const transactions: any = await executeQuery(`
        ...
        FROM rhlaporanpiutang h
        WHERE h.nomormhperusahaan = ? AND h.nomormhcustomer = ? 
          AND DATE(h.tanggal) BETWEEN ? AND ?
          AND h.nomormhcabang = ?
        ORDER BY h.tanggal ASC, h.transaksi_kode ASC
      `, [active_perusahaan, nomormhcustomer, startDate, endDate, active_cabang]);
      ...
      const results: any = await executeQuery(`
        SELECT h.transaksi_kode, h.tanggal, h.jatuh_tempo, c.nama as customer,
               h.total_idr as total, 
               h.total_idr as sisa_piutang
        FROM rhlaporanpiutang h
        LEFT JOIN mhcustomer c ON h.nomormhcustomer = c.nomor
        WHERE h.nomormhperusahaan = ? AND DATE(h.tanggal) <= ?
          AND h.nomormhcabang = ?
          AND h.total_idr <> 0
        ORDER BY c.nama ASC, h.tanggal ASC
      `, [active_perusahaan, endDate, active_cabang]);
      data = results.map((r: any) => ({
        ...r,
        total: parseFloat(r.total || 0),
        sisa_piutang: parseFloat(r.sisa_piutang || 0)
      }));

    } else if (type === 'mutasi') {
      if (!startDate) return NextResponse.json({ success: false, error: 'Tanggal Awal wajib diisi untuk Mutasi Piutang' }, { status: 400 });

      const results: any = await executeQuery(`
        SELECT 
          c.nama as customer,
          COALESCE((SELECT SUM(total_idr) FROM rhlaporanpiutang WHERE nomormhcustomer = c.nomor AND DATE(tanggal) < ? AND nomormhperusahaan = ? AND nomormhcabang = ?), 0) as saldo_awal,
          COALESCE((SELECT SUM(total_idr) FROM rhlaporanpiutang WHERE nomormhcustomer = c.nomor AND DATE(tanggal) BETWEEN ? AND ? AND total_idr > 0 AND nomormhperusahaan = ? AND nomormhcabang = ?), 0) as debet,
          COALESCE((SELECT ABS(SUM(total_idr)) FROM rhlaporanpiutang WHERE nomormhcustomer = c.nomor AND DATE(tanggal) BETWEEN ? AND ? AND total_idr < 0 AND nomormhperusahaan = ? AND nomormhcabang = ?), 0) as kredit,
          COALESCE((SELECT SUM(total_idr) FROM rhlaporanpiutang WHERE nomormhcustomer = c.nomor AND DATE(tanggal) <= ? AND nomormhperusahaan = ? AND nomormhcabang = ?), 0) as saldo_akhir
        FROM mhcustomer c
        WHERE EXISTS (SELECT 1 FROM rhlaporanpiutang WHERE nomormhcustomer = c.nomor AND nomormhperusahaan = ? AND nomormhcabang = ?)
        ORDER BY c.nama ASC
      ...
      const results: any = await executeQuery(`
        SELECT 
          ...
          SUM(h.total_idr) as total_piutang
        FROM rhlaporanpiutang h
        LEFT JOIN mhcustomer c ON h.nomormhcustomer = c.nomor
        WHERE h.nomormhperusahaan = ? AND DATE(h.tanggal) <= ?
          AND h.nomormhcabang = ?
    ...
    console.error("Report Piutang Error:", error);
