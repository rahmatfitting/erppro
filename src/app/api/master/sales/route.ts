...
export async function GET(request: Request) {
  try {
    ...
    let query = `SELECT * FROM mhsales WHERE 1=1`;
    const params: any[] = [];
    ...
    let countQuery = `SELECT COUNT(*) as total FROM mhsales WHERE 1=1`;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [rows]: any = await connection.execute(
      `SELECT kode FROM mhsales ORDER BY nomor DESC LIMIT 1 FOR UPDATE`
    );
    ...
    const query = `
      INSERT INTO mhsales (kode, nama, telepon, email, dibuat_oleh)
      VALUES (?, ?, ?, ?, ?)
    `;
    const params = [generatedKode, nama, telepon || '', email || '', 'Admin'];

    const [result]: any = await connection.execute(query, params);
    
    await connection.commit();
    return NextResponse.json({ success: true, message: "Sales berhasil ditambahkan", data: { nomor: result.insertId, kode: generatedKode } });
  } catch (error: any) {
...
export async function PATCH(request: Request) {
  try {
    ...
    const query = `UPDATE mhsales SET status_aktif = ? WHERE kode = ?`;
    await executeQuery(query, [status_aktif, id]);

    return NextResponse.json({ success: true, message: "Status sales berhasil diupdate" });
  } catch (error: any) {
