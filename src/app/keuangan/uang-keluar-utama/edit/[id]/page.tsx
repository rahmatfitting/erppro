...
import { Save, ArrowLeft, ArrowUpCircle, Plus, Trash2, AlertCircle, Search, Loader2 } from "lucide-react";
...
export default function UangKeluarUtamaEdit() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (d.success) router.push('/keuangan/uang-keluar-utama');
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-12">
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 flex-1"><ArrowUpCircle className="h-6 w-6 text-rose-600"/><h1 className="text-xl font-bold text-slate-900 dark:text-white">Edit UKU ({formData.account_nama})</h1></div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              ...
              :items.map((item,i)=>(
                <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  ...
                  <td className="px-3 py-2 text-slate-500 min-w-[100px]">{item.account_hutang || 'Hutang Usaha'}</td>
