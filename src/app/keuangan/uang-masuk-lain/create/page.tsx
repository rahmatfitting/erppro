...
import { Save, ArrowLeft, Plus, Trash2, AlertCircle, Search, ArrowDownCircle } from "lucide-react";
import { BrowseAccountModal } from "@/components/BrowseAccountModal";

const fmt = (v: number) => new Intl.NumberFormat('id-ID').format(v || 0);

export default function UangMasukLainCreate() {
  ...
  const addItem = () => setItems(prev => [...prev, { nomormhaccount: 0, account_kode: '', account_nama: '', nominal: 0, nominal_idr: 0, keterangan: '' }]);
  const removeItem = (i: number) => setItems(prev => prev.filter((_, idx) => idx !== i));
  const updateItem = (i: number, k: string, v: any) => setItems(prev => prev.map((it, idx) => {
    if (idx !== i) return it;
    const u = { ...it, [k]: v };
    if (k === 'nominal') u.nominal_idr = (parseFloat(v) || 0) * formData.kurs;
    return u;
  }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/keuangan/uang-masuk', { 
      ...
      body: JSON.stringify({ jenis: 0, ...formData, total: totalItems, total_idr: totalItems * (formData.kurs || 1), items }) 
    });
    const d = await res.json();
    if (d.success) router.push(`/keuangan/uang-masuk-lain/${d.data.id}`);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 flex-1"><ArrowDownCircle className="h-6 w-6 text-emerald-600" /><h1 className="text-xl font-bold text-slate-900 dark:text-white">Uang Masuk Lain (UML)</h1></div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
              ...
              {items.length === 0 ? <tr><td colSpan={6} className="px-3 py-6 text-center text-slate-400">Klik Tambah untuk menambah account</td></tr>
                : items.map((item, i) => (
                  <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    ...
                    <td className="px-3 py-2 text-slate-400 text-right w-24">{fmt(item.nominal_idr)}</td>
          ...
          <div className="p-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
            <button type="button" onClick={addItem} className="inline-flex items-center gap-1 px-3 py-1.5 bg-emerald-100 hover:bg-emerald-200 font-semibold text-emerald-700 text-xs rounded-lg transition-colors"><Plus className="h-3.5 w-3.5" /> Tambah Baris
            </button>
          </div>
        </div>
