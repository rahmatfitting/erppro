...
import { Save, ArrowLeft, ArrowDownCircle, Plus, Trash2, AlertCircle, Search } from "lucide-react";
...
const REF_TYPES = [
  { value: 'nota_jual', label: 'Nota Penjualan' },
  { value: 'ndc', label: 'Nota Debet Customer (NDC)' },
  { value: 'umc', label: 'Uang Muka Customer (UMC)' },
  { value: 'ums', label: 'Uang Muka Supplier (UMS)' },
];

export default function UangMasukUtamaCreate() {
  ...
  const [items, setItems] = useState<any[]>([]); // detail transaksi
  const [selisih, setSelisih] = useState<any[]>([]); // detail selisih
  ...
  const setField = (k: string, v: any) => setFormData(prev => {
    const u = { ...prev, [k]: v };
    if (k === 'total' || k === 'kurs') u.total_idr = (parseFloat(u.total?.toString() || '0') * parseFloat(u.kurs?.toString() || '1'));
    return u;
  });

  const addItem = () => setItems(prev => [...prev, { ref_jenis: '', ref_kode: '', ref_nomor: 0, account_piutang: 'Piutang Usaha', nominal_transaksi: 0, nominal_transaksi_idr: 0, total_terbayar: 0, total_terbayar_idr: 0, keterangan: '' }]);
  const removeItem = (i: number) => setItems(prev => prev.filter((_, idx) => idx !== i));
  const updateItem = (i: number, k: string, v: any) => setItems(prev => prev.map((it, idx) => {
     if (idx !== i) return it;
     const u = { ...it, [k]: v };
     if (k === 'total_terbayar') u.total_terbayar_idr = (parseFloat(v) || 0) * formData.kurs;
  ...
  const totalItems = items.reduce((s, it) => s + (parseFloat(it.total_terbayar) || 0), 0);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setLoading(true); setError(null);
    const res = await fetch('/api/keuangan/uang-masuk', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ jenis: 1, ...formData, total: totalItems, total_idr: totalItems * (formData.kurs || 1), items, selisih }) });
    const d = await res.json();
    if (d.success) router.push(`/keuangan/uang-masuk-utama/${d.data.kode}`);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-12">
      <div className="flex items-center gap-3">
        <Link href="/keuangan/uang-masuk-utama" className="p-2 -ml-2 rounded-full hover:bg-slate-100 text-slate-500"><ArrowLeft className="h-5 w-5" /></Link>
        <div className="flex items-center gap-2 flex-1"><ArrowDownCircle className="h-6 w-6 text-emerald-600" /><h1 className="text-xl font-bold text-slate-900 dark:text-white">Uang Masuk Utama (UMU)</h1></div>
        <button onClick={handleSubmit} disabled={loading} className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-lg disabled:opacity-50"><Save className="h-4 w-4" /> {loading ? 'Menyimpan...' : 'Simpan'}</button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
            <h2 className="text-sm font-bold text-slate-700 dark:text-slate-300">Detail Transaksi</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs whitespace-nowrap">
              <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                <tr>
                  {['#','Jenis Transaksi','Kode Transaksi','Account Piutang','Nominal Transaksi','Nominal IDR','Total Bayar','Total Bayar IDR','Ket',''].map(h=><th key={h} className="px-3 py-2 text-left font-semibold text-slate-600 text-xs">{h}</th>)}
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {items.length === 0 ? <tr><td colSpan={10} className="px-3 py-6 text-center text-slate-400">Klik Tambah untuk menambah transaksi</td></tr>
                : items.map((item, i) => (
                  <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    <td className="px-3 py-2 text-slate-500">{i+1}</td>
                    <td className="px-3 py-2"><select value={item.ref_jenis} onChange={e => { updateItem(i, 'ref_jenis', e.target.value); updateItem(i, 'ref_kode', ''); updateItem(i, 'ref_nomor', 0); }} className="text-xs px-2 py-1 border border-slate-200 dark:border-slate-600 rounded dark:bg-slate-900 dark:text-white focus:outline-none min-w-[140px]"><option value="">-- Pilih --</option>{REF_TYPES.map(r=><option key={r.value} value={r.value}>{r.label}</option>)}</select></td>
                    ...
                    <td className="px-3 py-2 text-slate-600 dark:text-slate-300 min-w-[120px]">{item.account_piutang || 'Piutang Usaha'}</td>
                    <td className="px-3 py-2"><input type="number" value={item.nominal_transaksi} onChange={e => updateItem(i, 'nominal_transaksi', parseFloat(e.target.value)||0)} className="text-xs px-2 py-1 border border-slate-200 dark:border-slate-600 rounded dark:bg-slate-900 dark:text-white focus:outline-none w-28 text-right" /></td>
                    <td className="px-3 py-2 text-slate-400 text-right w-24">{fmt(item.nominal_transaksi_idr)}</td>
                    <td className="px-3 py-2"><input type="number" value={item.total_terbayar} onChange={e => updateItem(i, 'total_terbayar', parseFloat(e.target.value)||0)} className="text-xs px-2 py-1 border border-slate-200 dark:border-slate-600 rounded dark:bg-slate-900 dark:text-white focus:outline-none w-28 text-right" /></td>
                    <td className="px-3 py-2 text-slate-400 text-right w-24">{fmt(item.total_terbayar_idr)}</td>
                    <td className="px-3 py-2"><input type="text" value={item.keterangan} onChange={e => updateItem(i, 'keterangan', e.target.value)} placeholder="Ket" className="text-xs px-2 py-1 border border-slate-200 dark:border-slate-600 rounded dark:bg-slate-900 dark:text-white focus:outline-none w-24" /></td>
          ...
          <div className="p-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
            <button type="button" onClick={addItem} className="inline-flex items-center gap-1 px-3 py-1.5 bg-emerald-100 hover:bg-emerald-200 text-emerald-700 text-xs font-semibold rounded-lg transition-colors"><Plus className="h-3.5 w-3.5" /> Tambah Baris</button>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
          ...
          <table className="w-full text-xs">
            <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
              <tr>{['#','Account','Nominal','Keterangan',''].map(h=><th key={h} className="px-3 py-2 text-left font-semibold text-slate-600">{h}</th>)}</tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              ...
              : selisih.map((s, i) => (
                <tr key={i}>
                  ...
                  <td className="px-3 py-2"><input type="number" value={s.nominal} onChange={e => updateSelisih(i, 'nominal', parseFloat(e.target.value)||0)} className="text-xs px-2 py-1 border border-slate-200 dark:border-slate-600 rounded dark:bg-slate-900 dark:text-white focus:outline-none w-28 text-right" /></td>
                  <td className="px-3 py-2"><input type="text" value={s.keterangan} onChange={e => updateSelisih(i, 'keterangan', e.target.value)} className="text-xs px-2 py-1 border border-slate-200 dark:border-slate-600 rounded dark:bg-slate-900 dark:text-white focus:outline-none w-32" /></td>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-5">
          <div className="max-w-xs ml-auto space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-slate-500">Total Bayar</span><span className="font-bold text-emerald-600">Rp {fmt(totalItems)}</span></div>
            <div className="flex justify-between text-slate-500 text-xs"><span>Total IDR (x{formData.kurs})</span><span>Rp {fmt(totalItems * (formData.kurs || 1))}</span></div>
          </div>
        </div>
      </form>
