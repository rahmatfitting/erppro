...
import { Save, ArrowLeft, ArrowDownCircle, Plus, Trash2, AlertCircle, Search, Loader2 } from "lucide-react";
...
export default function UangMasukUtamaEdit() {
      ...
      try {
        ...
        if (dataJson.success && dataJson.header) {
          ...
          setFormData({
            ...
            nomormhcustomer: h.nomormhcustomer,
            customer_nama: h.customer_nama || '',
            nomormhsupplier: h.nomormhsupplier,
            supplier_nama: h.supplier_nama || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/keuangan/uang-masuk', { 
      ...
      body: JSON.stringify({ id, jenis: 1, ...formData, total: totalItems, total_idr: totalItems * (formData.kurs || 1), items, selisih }) 
    });
    const d = await res.json();
    if (d.success) router.push('/keuangan/uang-masuk-utama');
    else { setError(d.error || 'Gagal'); setSaving(false); }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-12">
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 flex-1"><ArrowDownCircle className="h-6 w-6 text-emerald-600" /><h1 className="text-xl font-bold text-slate-900 dark:text-white">Edit UMU ({formData.account_nama})</h1></div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs whitespace-nowrap">
              <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                <tr>{['#','Jenis Transaksi','Kode Transaksi','Account Piutang','Nominal Transaksi','Nominal IDR','Total Bayar','Total Bayar IDR','Ket',''].map(h=><th key={h} className="px-3 py-2 text-left font-semibold text-slate-600 text-xs">{h}</th>)}</tr>
              </thead>
