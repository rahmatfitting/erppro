...
import { Save, ArrowLeft, Plus, Trash2, AlertCircle, Search, ArrowUpCircle, Loader2 } from "lucide-react";
import { BrowseAccountModal } from "@/components/BrowseAccountModal";

const fmt = (v: number) => new Intl.NumberFormat('id-ID').format(v || 0);

export default function UangKeluarLainEdit() {
    ...
    const fetchData = async () => {
      try {
        const [valutaRes, dataRes] = await Promise.all([
          ...
          fetch(`/api/keuangan/uang-keluar/detail?id=${id}`)
        ]);
        ...
        if (dataJson.success && dataJson.header) {
          ...
          setItems(dataJson.items.map((it: any) => ({
            ...
            nominal: it.nominal,
            total_bayar: it.total_bayar,
            total_bayar_idr: it.total_bayar_idr,
  ...
  const addItem = () => setItems(prev => [...prev, { nomormhaccount: 0, account_kode: '', account_nama: '', nominal: 0, total_bayar: 0, total_bayar_idr: 0, keterangan: '' }]);
  const removeItem = (i: number) => setItems(prev => prev.filter((_, idx) => idx !== i));
  const updateItem = (i: number, k: string, v: any) => setItems(prev => prev.map((it, idx) => {
    if (idx !== i) return it;
    const u = { ...it, [k]: v };
    if (k === 'nominal') {
        u.total_bayar = parseFloat(v) || 0;
        u.total_bayar_idr = u.total_bayar * formData.kurs;
    }
    return u;
  }));
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      <div className="flex items-center gap-3">
        <Link href="/keuangan/uang-keluar-lain" className="p-2 -ml-2 rounded-full hover:bg-slate-100 text-slate-500"><ArrowLeft className="h-5 w-5" /></Link>
        <div className="flex items-center gap-2 flex-1"><ArrowUpCircle className="h-6 w-6 text-rose-600" /><h1 className="text-xl font-bold text-slate-900 dark:text-white">Edit UKL ({formData.account_nama})</h1></div>
        <button onClick={handleSubmit} disabled={saving} className="inline-flex items-center gap-2 px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white text-sm font-medium rounded-lg disabled:opacity-50"><Save className="h-4 w-4" /> {saving ? 'Menyimpan...' : 'Simpan Perubahan'}</button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
              ...
              {items.length === 0 ? <tr><td colSpan={6} className="px-3 py-6 text-center text-slate-400">Klik Tambah untuk menambah account</td></tr>
                : items.map((item, i) => (
                  <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    ...
                    <td className="px-3 py-2 text-slate-400 text-right w-24">{fmt(item.total_bayar_idr)}</td>
          ...
          <div className="p-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
            <button type="button" onClick={addItem} className="inline-flex items-center gap-1 px-3 py-1.5 bg-rose-100 hover:bg-rose-200 font-semibold text-rose-700 text-xs rounded-lg transition-colors"><Plus className="h-3.5 w-3.5" /> Tambah Baris</button>
          </div>
        </div>

        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-4">
          <div className="max-w-xs ml-auto space-y-1 text-sm">
            <div className="flex justify-between"><span className="text-slate-500 font-medium">Total</span><span className="font-bold text-rose-600">Rp {fmt(totalItems)}</span></div>
