...
const REPORT_TYPES = [
  { id: 'kartu', title: 'Kartu Hutang Supplier', desc: 'Buku pembantu hutang per supplier untuk melihat mutasi rincian.', icon: Receipt, color: 'text-blue-600', bg: 'bg-blue-50', showSupplier: true },
  { id: 'posisi', title: 'Posisi Hutang', desc: 'Saldo hutang yang masih beredar (outstanding) per tanggal.', icon: Archive, color: 'text-indigo-600', bg: 'bg-indigo-50' },
  { id: 'mutasi', title: 'Mutasi Hutang', desc: 'Ringkasan saldo awal, debet, kredit dan saldo akhir per supplier.', icon: BarChart3, color: 'text-emerald-600', bg: 'bg-emerald-50' },
  { id: 'aging', title: 'Aging Hutang', desc: 'Analisis umur hutang berdasarkan tanggal jatuh tempo.', icon: Clock, color: 'text-amber-600', bg: 'bg-amber-50' },
];

export default function ReportHutangPage() {
  ...
  const handleFilter = async (startDate: string, endDate: string, exportType: 'view' | 'pdf' | 'excel' | 'pivot', options: any = {}) => {
    ...
    try {
      ...
      let url = `/api/report/hutang?type=${selectedReport.id}&startDate=${startDate}&endDate=${endDate}`;
      if (options.nomormhsupplier) url += `&nomormhsupplier=${options.nomormhsupplier}`;
  ...
  return (
    <div className="p-6 space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Laporan Hutang</h1>
      ...
      {selectedReport && (
        <ReportFilterModal
          ...
          showSupplier={selectedReport.showSupplier}
