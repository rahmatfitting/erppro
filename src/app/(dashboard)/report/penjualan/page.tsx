...
import { FileText, Receipt, SearchCheck, Truck, Loader2, AlertCircle } from "lucide-react";
...
const REPORT_TYPES = [
  { id: 'rekap', title: 'Laporan Rekap Penjualan', desc: 'Ringkasan total per nota penjualan.', icon: Receipt, color: 'text-blue-600', bg: 'bg-blue-50' },
  { id: 'detail', title: 'Laporan Detail Penjualan', desc: 'Rincian barang per nota penjualan.', icon: FileText, color: 'text-indigo-600', bg: 'bg-indigo-50' },
  { id: 'monitoring', title: 'Monitoring Order Jual', desc: 'Lacak status SO ke Surat Jalan hingga Nota Jual.', icon: SearchCheck, color: 'text-emerald-600', bg: 'bg-emerald-50' },
  { id: 'outstanding', title: 'Outstanding Surat Jalan', desc: 'Daftar Surat Jalan yang belum dibuatkan Nota Jual.', icon: Truck, color: 'text-amber-600', bg: 'bg-amber-50' },
];

export default function ReportPenjualanPage() {
  ...
  const handleFilter = async (startDate: string, endDate: string, exportType: 'view' | 'pdf' | 'excel' | 'pivot', page = 1) => {
    setLoading(true);
    setError(null);
    try {
      ...
      const res = await fetch(`/api/report/penjualan?type=${selectedReport.id}&startDate=${startDate}&endDate=${endDate}&limit=${limit}&offset=${offset}`);
    ...
    <div className="p-6 space-y-6 animate-in fade-in duration-500">
      ...
      {!loading && reportData.length > 0 && (
         <div className="mt-8 space-y-4 animate-in slide-in-from-bottom duration-500">
            ...
            <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm shadow-indigo-100 dark:shadow-none">
            ...
            {totalCount > 50 && lastFilter && (
                <div className="flex items-center justify-between p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm shadow-indigo-100 dark:shadow-none">
