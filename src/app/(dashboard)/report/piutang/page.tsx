...
const REPORT_TYPES = [
  { id: 'kartu', title: 'Kartu Piutang Customer', desc: 'Buku pembantu piutang per customer untuk melihat mutasi rincian.', icon: Receipt, color: 'text-blue-600', bg: 'bg-blue-50', showCustomer: true },
  { id: 'posisi', title: 'Posisi Piutang', desc: 'Saldo piutang yang masih beredar (outstanding) per tanggal.', icon: Archive, color: 'text-indigo-600', bg: 'bg-indigo-50' },
  { id: 'mutasi', title: 'Mutasi Piutang', desc: 'Ringkasan saldo awal, debet, kredit dan saldo akhir per customer.', icon: BarChart3, color: 'text-emerald-600', bg: 'bg-emerald-50' },
  { id: 'aging', title: 'Aging Piutang', desc: 'Analisis umur piutang berdasarkan tanggal jatuh tempo.', icon: Clock, color: 'text-amber-600', bg: 'bg-amber-50' },
];

export default function ReportPiutangPage() {
  ...
  const handleFilter = async (startDate: string, endDate: string, exportType: 'view' | 'pdf' | 'excel' | 'pivot', options: any = {}) => {
    ...
    try {
      ...
      let url = `/api/report/piutang?type=${selectedReport.id}&startDate=${startDate}&endDate=${endDate}`;
      if (options.nomormhcustomer) url += `&nomormhcustomer=${options.nomormhcustomer}`;
      ...
      if (isExport) {
        ...
        setLastFilter({ startDate, endDate, options });
      }
  ...
  return (
    <div className="p-6 space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Laporan Piutang</h1>
      ...
      {selectedReport && (
        <ReportFilterModal
          ...
          showCustomer={selectedReport.showCustomer}
