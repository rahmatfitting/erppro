...
import { Users, BookOpen, Save, Loader2, Search, CheckCircle2, ShieldCheck, Wallet } from "lucide-react";
import { cn } from "@/lib/utils";

export default function SettingAksesAccount() {
  ...
  const fetchAccounts = async () => {
    try {
      const res = await fetch('/api/master/account?limit=999');
      const json = await res.json();
  ...
  const fetchUserAkses = async (userId: number) => {
    ...
    try {
      const res = await fetch(`/api/setting/user-akses/account?userId=${userId}`);
      const json = await res.json();
      if (json.success) {
        setUserAkses(json.data.map((item: any) => item.nomormhaccount));
      }
    } catch (e) { console.error(e); }
  ...
  const selectAll = () => {
    setUserAkses(accounts.map(a => a.nomor));
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch('/api/setting/user-akses/account', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: selectedUserId, accountIds: userAkses })
      });
      const json = await res.json();
      if (json.success) {
        alert("Akses account berhasil disimpan");
      } else {
        alert(json.error);
      }
    } catch (e) {
  ...
  const filteredAccounts = accounts.filter(a => 
    a.nama.toLowerCase().includes(searchAccount.toLowerCase()) || 
    a.kode.toLowerCase().includes(searchAccount.toLowerCase())
  );
    ...
    <div className="p-6 space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2 uppercase tracking-tighter">
          ...
          Setting Akses Account User
        </h1>
        <p className="text-slate-500 dark:text-slate-400">Pilih akun apa saja yang bisa digunakan oleh tiap-tiap user.</p>
      </div>
             ...
             <div className="p-6 bg-indigo-600 rounded-2xl text-white shadow-xl shadow-indigo-200 dark:shadow-none animate-in zoom-in duration-300">
                ...
                <div className="p-3 bg-white/10 rounded-xl space-y-2">
                    <div className="flex justify-between text-xs font-bold">
                        <span className="text-indigo-200 font-medium">Account Terpilih</span>
                        <span>{userAkses.length} / {accounts.length}</span>
                    </div>
                    <div className="w-full bg-white/10 h-1.5 rounded-full overflow-hidden">
                        <div 
                            className="bg-white h-full transition-all duration-500" 
                            style={{ width: `${(userAkses.length / accounts.length) * 100}%` }}
                        />
                    </div>
                </div>
                <button
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
            <div className="p-4 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-bold text-slate-800 dark:text-white uppercase text-xs tracking-widest">Daftar Account / Kas Bank</h2>
              </div>
            ...
            <div className="p-4 border-b border-slate-50 dark:border-slate-800">
               <div className="relative">
                  ...
                  <input 
                    type="text" 
                    placeholder="Cari account berdasarkan kode atau nama..."
                    value={searchAccount}
              ...
              {!selectedUserId ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {filteredAccounts.map(a => {
                    const isSelected = userAkses.includes(a.nomor);
                    return (
                      <div 
                        key={a.nomor}
                        onClick={() => toggleAccount(a.nomor)}
                        className={cn(
                      ...
                      >
                         <div className="flex items-center gap-3">
                            ...
                            <div>
                               <div className="text-[10px] font-black text-slate-400 uppercase tracking-tighter leading-none mb-1 group-hover:text-indigo-400">{a.kode}</div>
                               <div className={cn("text-sm font-black uppercase tabular-nums", isSelected ? "text-indigo-900 dark:text-white" : "text-slate-700 dark:text-slate-300")}>{a.nama}</div>
                            </div>
                         </div>
